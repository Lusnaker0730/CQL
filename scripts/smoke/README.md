...
- `groups: [{groupId, populations, score?, scoreTolerance?}]` — multi-group assertion (no scenario currently uses this; reserved for when backend multi-group evaluation lands — see issue #474). Each entry matches a response group by `groupId` and asserts populations + optional score. Falls back to top-level `populations`/`score` for single-group scenarios.
