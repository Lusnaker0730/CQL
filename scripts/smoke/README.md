...
| `39-library-function-call` | Library function calls with arguments (PAT-237) | Scenario 33's five patients; the eCQM's populations are `externalCqlFunctionCall` elements calling functions of an uploaded `SmokeFnUtil` library with every argument source the builder offers — IP `AgeAtLeast(18)` (literal Integer), denominator `AgeBetweenAsOf(50, 120, @2024-06-30)` (literal Integers + Date), numerator `AtLeastAtPeriodEnd("Measurement Period", "Senior Threshold")` (Measurement Period + an artifact parameter defaulting to 65) AND `Identity("Is Adult")` (a base element). Same numbers as 33 (4 / 3 / 2, 66.67), then the `measure-package` round trip: `package.cqlContains` locks the exact call text, include and parameter declaration in the packaged primary CQL; the function library travels as a `depends-on` Library and the re-imported measure evaluates identically. |
| `38-component-stratifier` | Multi-component stratifier (PAT-235) | Scenario 19's seven patients and proportion measure with the classic boolean `gender` stratifier plus one artifact-level **multi-component** stratifier `sex-age` = sex (`Patient.gender.value`) × age band (18-49 / 50-64 / 65+), as the eCQM workspace builds it. Locks: one define per component (`Stratifier sex-age sex`, `Stratifier sex-age age`); a patient's stratum is the combination (`female | 50-64`) reported with per-component values (`components` assertion per stratum); the empty male 50-64 cell is no stratum (`exactStrata` = exactly the five combinations that occur); per-stratum populations and scores. Runs on PostgreSQL with V74 (`measure_report_stratifier.component_values`) applied by Flyway. |
| `37-supplemental-data-distributions` | Supplemental data / risk adjustment factors (PAT-234) | Scenario 19's seven patients and proportion measure with three supplemental data elements as the eCQM workspace's SDE tab builds them: the standard `SDE Sex`, a value-kind risk adjustment factor `RAF Age Band` (age bands 18-49 / 50-64 / 65+) and a boolean risk adjustment factor `RAF Elderly` (the numerator's AgeRange ≥ 65 tree). Locks: the workspace's SDE elements reach the published `MeasureDefinition` by usage (publish used to drop them all); the evaluation reports each element as a value → patient-count distribution (`female: 4, male: 3`; `SDE Sex` used to contribute nothing because strings were dropped by the count-only aggregation); a FHIR primitive counts as its value; a boolean element distributes as `true` / `false`; patients without a value are counted separately. Runs on PostgreSQL with V73 (`measure_report_supplemental_data`, RLS via `measure_report`) applied by Flyway under the app role — the report persistence check would fail if the distribution rows could not be written. |
...
- `package.cqlContains[]` (scenario `39`, PAT-237) — exact fragments the packaged primary Library's decoded CQL must contain (`grep -F`), e.g. a library function call with its arguments.
- `package.standardMetadata` (scenario `33`, PAT-236) — `{measureTypes[], experimental, effectiveStart, effectiveEnd, approvalDate, lastReviewDate, clinicalRecommendationStatement, definitions[]}`, all optional. `assert-measure-package.sh` checks each on the exported Measure as its R4 element (`type[].coding[].code`, `effectivePeriod.start/end`, `definition[]` markdown entries exactly) **and** on the imported MeasureDefinition (`import-result.json`), so the round trip covers the metadata too.
- `stratifiers[]` (used by scenarios `19`, `36`) — array of `{strataId, expectedStrata: [{strataValue, populations, score?}], scoreTolerance?}`. Each entry asserts a stratifier produced the expected strata; strata are matched by `strataValue` (response order doesn't matter); `score` is optional per stratum (omit it for a stratum whose denominator is empty); `components: {code: value}` (PAT-235) additionally asserts the exact per-component values of a multi-component stratum. Top-level `exactStrata: true` (PAT-233) additionally asserts each listed stratifier has **exactly** the listed strata — for value stratifiers, so no stray `null` / type-name bucket goes unnoticed.
- `supplementalDataResults[]` (PAT-234, used by scenario `37`) — array of `{definition, usage, values: {value: count, …}, patientsWithoutValue}`. Each entry asserts one declared supplemental data element / risk adjustment factor's usage, its **exact** value → patient-count map and the number of patients without a value.
