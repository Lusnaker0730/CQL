...
- `groups: [{groupId, populations, score?, scoreTolerance?}]` (used by scenario `25`) — multi-group assertion. Each entry matches a response group by `groupId` and asserts populations + optional score. Use for measures with more than one population group; falls back to top-level `populations`/`score` for single-group scenarios.
- `maxEvaluationTimeMs: 8000` (used by scenarios `01`, `19`, `21`-`25`) — wall-clock budget around the `$evaluate-measure` call. Run.sh times the evaluate call and exports `EVAL_ELAPSED_MS`; assert.sh fails the scenario if elapsed exceeds the budget. Catches N+1 query regressions and bulk-fetch slowdowns. Budget intentionally generous (typical eval runs in 1-2s) — guards against 4-8x regressions, not microbenchmarks.
