...
public class EmailService {
    ...
    public void sendTemporaryPasswordEmail(String toEmail, String username, String temporaryPassword) {
        try {
            ...
            helper.setSubject("台灣CQL概念平台 - Temporary Password");
            helper.setText(buildTemporaryPasswordEmailHtml(username, temporaryPassword), true);

            mailSender.send(message);
            log.info("Temporary password email sent to user: {}", username);
        } catch (MessagingException e) {
            throw new RuntimeException("Failed to send temporary password email to user: " + username, e);
        }
    }

    private String buildTemporaryPasswordEmailHtml(String username, String temporaryPassword) {
        return """
            <!DOCTYPE html>
            <html>
            <head>
                ...
                <table width="100%%" cellpadding="0" cellspacing="0" style="max-width:600px;margin:40px auto;">
                    <tr>
                        <td style="background:linear-gradient(135deg,#0D7377,#1B3A5C);padding:32px;text-align:center;border-radius:12px 12px 0 0;">
                            ...
                            <p style="color:rgba(255,255,255,0.8);margin:8px 0 0;font-size:14px;">Temporary Password</p>
                        </td>
                    </tr>
                    <tr>
                        <td style="background:white;padding:32px;border-radius:0 0 12px 12px;box-shadow:0 2px 8px rgba(0,0,0,0.1);">
                            ...
                            <table width="100%%" cellpadding="0" cellspacing="0" style="margin:24px 0;">
                                <tr>
                                    <td align="center">
                                        <div style="display:inline-block;padding:14px 32px;background:#f4f7f9;border:2px solid #0D7377;border-radius:8px;font-family:monospace;font-size:18px;letter-spacing:2px;color:#1B3A5C;">
                                            %s
                                        </div>
                                    </td>
                                </tr>
