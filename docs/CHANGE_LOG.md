...
|-----|------|------|------|------|------|--------|
| PAT-170 | ✨ feat | 2026-06-02 | 前端 SEO（新 `frontend/public/og-image.png` + `frontend/index.html` 加 og:image/twitter:image + 修 html lang） | **SEO 補強:社群分享縮圖（og:image）+ 修正 html lang** — SEO 盤點發現兩缺口:(1) 完全沒 `og:image` → LINE/FB/Twitter 分享無預覽縮圖（對 LINE 分享為主的台灣使用者最有感）;(2) `<html lang="en">` 但站台主要內容為繁中（`og:locale=zh_TW`），語言訊號不符。**修補**:(a) 以 PIL + 微軟正黑產 **1200×630 `og-image.png`**（品牌色 teal `#0D7377`／深藍 `#1B3A5C` 漸層 + 平台名 + tagline + 醫療十字浮水印），置 `frontend/public/`（Vite build 自動進 dist 根、nginx `try_files` 直接提供）;(b) `index.html` 加 `og:image`（+`width`/`height`/`alt`）、`twitter:image`，`twitter:card` 由 `summary` → `summary_large_image`;(c) `<html lang="en">` → `zh-Hant-TW`。per-page Helmet（LandingPage/LearnPage）不需改 — 預設繼承 index.html 的 og:image。**未納入**（SEO 盤點其餘缺口，待產品定位決定）:JSON-LD 結構化資料、`/learn` prerender、hreflang、登入後切英文時動態同步 `documentElement.lang`。**TFDA 追溯**:#583（安全性等級 A — 前端 meta/靜態資產，不涉應用邏輯、臨床決策或患者資料）。 | frontend/index.html, frontend/public/og-image.png | |
