...
|-----|------|------|------|------|------|--------|
| PAT-215 | 🔒 fix | 2026-07-23 | 清除擋 Security Scan gate 的既有依賴 CVE:pgjdbc 42.7.11→42.7.12 + pin jackson-core 2.x 2.21.4(`backend/pom.xml`) | **修兩個既有依賴漏洞、恢復 Trivy Security Scan gate 綠燈** —(1)`org.postgresql:postgresql` 42.7.11 → **42.7.12**(CVE-2026-54291,經 `com.ongres.scram/scram-client`);(2)`com.fasterxml.jackson.core:jackson-core`(2.x)2.21.2 → **2.21.4**(GHSA-r7wm-3cxj-wff9,Async parser maxNumberLength bypass)—— Spring Boot 4.0.6 BOM 把 jackson-core 壓在 2.21.2,既有 `jackson-databind` 直接依賴 pin 2.21.4 不會連帶提升 core,故補一條 jackson-core 直接依賴 pin。兩者皆**既有依賴問題(非功能變更引入)**,會擋所有 PR 的 Security Scan gate,於 #817(BUG-141)重跑時被 429 flake 消退後逐一揭露。既有修復版可用故採升版而非 `.trivyignore` VEX 忽略(沿用 42.7.10→42.7.11 修 CVE-2026-42198 慣例)。**純依賴版本 bump,無 schema / migration / 應用程式碼變更**;42.7.12 與 jackson-core 2.21.4 於 Maven Central 皆存在(jar 200),CI Backend Tests / Docker Build / PostgreSQL Migration Test 驗證資料層不受影響。**TFDA 追溯**:#818(安全性等級 A,純依賴升版、無 PHI/邏輯變更)。 | 清 pgjdbc + jackson-core CVE;恢復 Security Scan gate;無 migration | |
