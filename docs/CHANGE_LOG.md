...
|-----|------|------|------|------|------|--------|
| PAT-104 | 🧪 tooling | 2026-04-23 | 測試（LibraryDefinitionPicker 全面覆蓋） | **Library picker 完整測試集：hook + 元件 + smoke scenario** — PAT-103 上線後列 3 個 follow-up 測試：(1) RTL for ArtifactWorkspace picker integration、(2) authoring smoke scenario、(3) LibraryDefinitionPicker component tests。本 PR 全部做完。(1) 新 `useArtifactLibraryPicker` hook 把 picker state + dispatch 邏輯從 `ArtifactWorkspace.tsx` 抽出，讓它在 isolation 裡可測；`ArtifactWorkspace.tsx` 對應更新為使用 hook（減 15 行 inline state）。新 `hooks/__tests__/useArtifactLibraryPicker.test.ts` 8 tests：initial null target / openForInclude / openForExclude / close without dispatch / handleSelect with include → 呼叫 onAddIncludeElement 且不碰 exclude / handleSelect with exclude ditto / defensive no-target handleSelect no-op / sequential include-then-exclude 兩個 handler 各被呼叫一次不交叉。8/8 pass in 57ms。(2) 新 smoke scenario 22 `22-authoring-library-reference`：新 scenario type `authoring-cql`（`run.sh` dispatch）。POST `ArtifactRequest` 含 `externalCqlElement` 節點到 `/api/authoring/artifacts`，follow-up `POST /artifacts/{id}/cql`，斷言 `cqlContains` 含 `include SharedLogic version '1.2.0' called shared` + `\"shared\".\"HasDiabetes\"` body reference，`cqlDoesNotContain` 含 `externalCqlElement`（防 tree-node marker leak）。新 helpers `lib/generate-authoring-cql.sh` + `lib/assert-authoring-cql.sh`，README 更新。全 20/20 smoke pass。(3) 新 `LibraryDefinitionPicker.test.tsx` 11 tests：`generateAlias` pure-function 5 tests（PascalCase / snake_case / kebab-case / 單字 / 空字串）+ dialog 6 tests（filter retired libs / 點選 library 進 step 2 且 alias 自動填 / 點 Select 發出 LibraryDefinitionReference 正確四欄 / Select 按鈕在缺 definition 或 alias 時 disabled / Back 回 step 1 不 emit / no-definitions case 不崩）。Mock `cqlApi.getLibraries` + `useCqlLibrary` + `useDebouncedValue`。`generateAlias` 從 internal function 改 export 讓測試讀取。本機 vitest 受 Windows EMFILE 影響（`@mui/icons-material` barrel）執行不完，Linux CI 會跑完整 11 tests；hook tests 不受此影響本機已 8/8 綠。TSC noEmit 零 error。全 backend 不受影響（無 backend 改動）。TFDA 追溯：需求 #TBD（安全性等級 A — testing infrastructure） | frontend/src/hooks/useArtifactLibraryPicker.ts, frontend/src/hooks/\_\_tests\_\_/useArtifactLibraryPicker.test.ts, frontend/src/components/authoring/ArtifactWorkspace.tsx, frontend/src/components/cql-libraries/LibraryDefinitionPicker.tsx, frontend/src/components/cql-libraries/\_\_tests\_\_/LibraryDefinitionPicker.test.tsx, scripts/smoke/scenarios/22-authoring-library-reference, scripts/smoke/lib/generate-authoring-cql.sh, scripts/smoke/lib/assert-authoring-cql.sh, scripts/smoke/run.sh, scripts/smoke/README.md | |
