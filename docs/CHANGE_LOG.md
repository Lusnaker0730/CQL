...
| PAT-224 | ✨ patch | 2026-09-17 | 後端 Java 21 → 25 執行環境升版 + `jacoco-maven-plugin` 0.8.14（`backend/pom.xml`、`ci.yml` / `deploy.yml` / `regulatory-docs.yml`、`docker/Dockerfile.backend`、README / DEPLOYMENT_GUIDE / backend/CLAUDE.md） | **收尾 VS Code Java 升版 agent（appmod）2026-08-27 的分支** — agent 只做到「改設定 + 編譯成功」（commit 訊息無 Tests 欄、未跑測試、未推遠端），本次驗證完成並補漏。**變更**：`<java.version>` 21 → 25、三個 workflow 的 `setup-java` 25、建置映像 `maven:3.9-eclipse-temurin-25`、執行映像 `eclipse-temurin:25-jre-alpine`、文件版本字樣；**新增 `jacoco-maven-plugin` 0.8.12 → 0.8.14**（0.8.12 不支援 Java 25 class 檔 major version 69、0.8.13 僅實驗性、0.8.14 正式支援；不升 CI 覆蓋率 agent 會炸）。**已查證**：Spring Boot 4.1 系統需求相容至 Java 26；兩個 temurin-25 映像存在於 Docker Hub；本機 JDK 25.0.2 `--release 25` fresh 編譯成功、class 檔 major 69。**驗證**：本機 JDK 25.0.2 完整後端套件 **1781 案、0 失敗、0 錯誤、1 略過**（PG-gated 整合測試，CI migration-test 跑）、jacoco 0.8.14 `prepare-agent` / `report` 正常（Analyzed 247 classes）、4 分 56 秒；CI Docker Build（temurin-25）+ Integration Smoke 31（25-jre-alpine）+ Migration Test 由 PR 驗。無 schema / migration / 程式碼變更；回退只需改回 21。正式機映像自帶 JRE，VM 不需裝 JDK。 | 需求 #856（A 級）；無 migration |[`6738bc5`](../../commit/6738bc5) |
