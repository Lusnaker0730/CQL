...
|-----|------|------|------|------|------|--------|
| BUG-120 | 🐛 fix | 2026-05-31 | 監控設定（移除 `docker/alertmanager.yml`、`docker-compose.yml` alertmanager service、`prometheus.yml` `alerting:` 區塊）+ smoke override（`scripts/smoke/compose.override.yml` 移除 alertmanager disabled stub）+ 文件（`README.md`、`DEPLOYMENT_GUIDE_zh-TW.md` §8.3） | **修復 production VM「假 ServiceDown 告警 + alertmanager 死信噪音」並移除從未生效的 Alertmanager** — VM Docker log 巡檢發現:應用容器（backend/hapi/postgres）一週內 0 錯誤，唯一一堆錯誤是 alertmanager 今天 00:00–05:21 共 64 筆 `Notify for alerts failed ... dial tcp [::1]:9095: connect: connection refused`。逐段查證根因鏈:(1) **根因 A — 假告警**:Prometheus scrape `backend:8080/actuator/prometheus` 回 **401**，`up{job=cql-platform}=0` → 觸發 `ServiceDown` critical 告警，**但 backend `RestartCount=0`、healthy、凌晨整段正常記 log，是假告警**。再往下查 401 來源:PAT-157 輪替了 `METRICS_SCRAPE_PASSWORD`（backend 6 天前重啟已吃到新值 `PNGN…`），但 **prometheus 容器「Up 5 weeks」從未重啟，仍持舊密碼 `ON1j…`**（username `prometheus` 兩邊一致、僅密碼一新一舊）→ basic-auth 失敗回 401。**處置 A（VM 即時，無 code 變更）**:`docker compose up -d --no-deps --force-recreate prometheus` 讓容器重讀 `.env`、重寫 `/tmp/scrape_password`；驗證 target `health=up`、`lastError=""`、active alerts `[]`，假告警自動 resolve。(2) **根因 B — Alertmanager 死設定**:三個 receiver（default/pager/slack）全寫死 `http://localhost:9095/{webhook,pager,slack}`，repo 內無任何服務或容器在 9095（git 溯源為早期 `phase 4/5` scaffold commit、無 changelog、無實作 code），即「就算告警是真的也送不出去」的 placeholder。**處置 B（本 PR）**:移除 alertmanager service + config + prometheus `alerting:` 區塊；告警規則（`prometheus-alerts.yml`）保留，仍由 Prometheus 自身評估、於 `:9090/alerts` 可見，僅不再外推。日後要恢復通知於 `prometheus.yml` 補回 `alerting:` 區塊 + 真實 receiver（Slack/Email）即可。**為何不過度修**:未替使用者選定告警目的地（需產品決策）；合規文件（HIPAA/ISO27001/pen-test）對 alertmanager 的描述未在本 PR 變更，另行向維運確認。**TFDA 追溯**:BUG（安全性等級 A — 純維運/監控設定，不改動應用回應、不涉臨床決策或患者資料）。 | docker/docker-compose.yml, docker/prometheus.yml, docker/alertmanager.yml（刪除）, scripts/smoke/compose.override.yml, README.md, DEPLOYMENT_GUIDE_zh-TW.md | |
