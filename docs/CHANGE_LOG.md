...
|-----|------|------|------|------|------|--------|
| BUG-123 | 🐛 fix | 2026-06-01 | CI（`.github/workflows/ci.yml` `security-scan` job 加 setup-java Maven 快取 + `dependency:go-offline` 預解析） | **修 CI Security Scan 間歇性失敗 — backend Trivy 被 Sonatype 429 限流 FATAL** — `security-scan` job 的 backend Trivy fs 掃描間歇性 `FATAL: remote Maven repository returned 429 Too Many Requests for oss.sonatype.org/.../cql-to-elm-jvm-4.5.0.pom`（PR #578 即中此 flake）。根因:Trivy 的 pom.xml 分析器要抓 POM 解析相依樹，`security-scan` job 沒有本地 Maven 快取 → Trivy 對 oss.sonatype.org（專案為 cqframework 宣告的 repo）發大量請求被 **429 限流封 30 分鐘** → 整個 job FATAL。**與程式碼/相依漏洞無關**,純 CI 基礎設施 flake,隨機把 PR 弄紅。**修補（採 Trivy 錯誤訊息自身建議:掃描前 populate `~/.m2`）**:`security-scan` 已 `needs: backend-test`（後者 `cache: 'maven'` 跑過 `mvn test`、快取已存），於 backend Trivy 步驟前加 (1) `setup-java cache: 'maven'` 還原該快取、(2) `mvn dependency:go-offline || true` 確保齊全 → Trivy 改讀本地 `~/.m2`,不再打 Sonatype。**驗證**:`yaml.safe_load` 確認 job 步驟順序（checkout → setup-java → go-offline → Trivy backend → Trivy frontend）;後續 PR 的 Security Scan 應穩定綠。**TFDA 追溯**:#579（安全性等級 A — CI workflow 修補,不改應用邏輯、不涉臨床決策或患者資料）。 | .github/workflows/ci.yml | |
