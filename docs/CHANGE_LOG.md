...
|-----|------|------|------|------|------|--------|
| BUG-124 | 🐛 fix | 2026-06-02 | 後端相依（`backend/pom.xml` `hapi.fhir.version` `8.8.1→8.10.0` + 新 `fhir-core.version=6.9.9` 把全部 12 個 `org.hl7.fhir.*` pin 6.9.9）+ `backend/.trivyignore` 移除 HAPI 4 個 CVE | **HAPI FHIR 8.8.1→8.10.0 + org.hl7.fhir.* 全 pin 6.9.9 — 根治 BUG-123 VEX 的 r4 CVE** — BUG-123 把 `org.hl7.fhir.r4` 6.7.9 的 `CVE-2026-45367`（FHIRPath ReDoS）VEX 掉，因當時 r4/model/support/utilities/validation 這組「緊耦合」套件無法獨立升（6.7.9→6.9.7 跨 minor 會 `AbstractMethodError`）。**查證**:HAPI 最新 = **8.10.0**（無 9.x）;8.8.1 bundle fhir_core 6.7.9、8.10.0 bundle 6.9.4.1;CVE fixed in 6.9.7、org.hl7.fhir.* 已發佈到 6.9.9;`dependency:tree` 顯示專案拉 12 個 org.hl7.fhir.*（6 已 6.9.7、6 仍 6.7.9）。**作法**:(1) `hapi.fhir.version` → 8.10.0;(2) 新 `fhir-core.version=6.9.9`，dependencyManagement 把**全部 12 個** org.hl7.fhir.* pin 之（同 6.9.x 線、與 HAPI 8.10.0 的 6.9.4.1 ABI 相容 → 舊的 6.7→6.9 skew 限制不再適用、無版本歧義）;(3) `.trivyignore` 移除 HAPI 4 個 CVE（33180/34359/34361/45367，皆由 6.9.9 修），只留 libthrift。**驗證**:VM `dependency:tree` 確認 12 個全解析 6.9.9 無衝突;**Backend Tests + 本機 smoke test 驗 runtime**（FhirContext/validation/IG/terminology/r4 model 共 26 檔用 HAPI，6.9.x 內升版若仍 ABI 不相容會 AbstractMethodError）;Security Scan 應只剩 libthrift VEX。**退路**:若 runtime 爆 → 退回「只升 8.10.0 + 保留 45367 VEX」。**TFDA 追溯**:#581（安全性等級 A — 相依安全升版，不改應用邏輯）。 | backend/pom.xml, backend/.trivyignore | |
