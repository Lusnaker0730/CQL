...
|-----|------|------|------|------|------|--------|
| PAT-238 | ✨ feat | 2026-09-24 | 後端（V76 + rollback、`entity/EcqmArtifactEntity`（`publishedAt` / `publishedContentHash`）、新增 `service/ecqm/PublishedContent`、`model/ecqm/BuilderSource`、`exception/PublishConflictException`（+ `GlobalExceptionHandler` 409）、`service/ecqm/EcqmPublishService`（`publish(…, force)` / `measureEditedSincePublish` / `builderSourceOf`）、`repository/EcqmArtifactRepository`、`controller/EcqmController`（`?force`）/ `MeasureController`（`GET /{id}/builder-source`）、`EcqmArtifactResponse.publishedAt`）、前端（新增 `measure/BuilderSourceBanner`、`ecqm/PublishConflictDialog`、`utils/publishConflict.ts`；`MeasureEditor`、`EcqmArtifactWorkspace` / `EcqmCqlPreviewTab` / `EcqmArtifactWorkspaceHeader`、`pages/EcqmPage` / `MeasuresPage`（deep link）、`api/ecqmApi` / `measureApi`、`hooks/useEcqm`、`types/ecqm`、`locales/*/{measures,ecqm}.json`）、`docs/API.md`、`CLAUDE.md` ×3 | **builder ↔ 指標雙向（eCQM 缺口第 9 項）** — 以前 builder → publish → 指標是單向的：指標頁不知道自己是 builder 建的、回不去；在指標頁改過的邏輯會在下一次從 builder publish 時**被無聲覆寫**。現在：publish 在 artifact 記下時間與內容指紋（V76：`published_at`、`published_content_hash` = CQL + group definitions 正規化後 SHA-256，撐得過 JSON 欄位的 DB 往返）。**指標頁**：`BuilderSourceBanner` 顯示來源 artifact、擁有者、發布時間與「在 builder 開啟」（`/ecqm?artifact=<id>`），並說明兩邊漂移——指標在這裡被改過（下次 publish 會先問）、builder 有未發布的變更。**builder**：header 的已發布 chip 連到指標（`/measures?measure=<id>`）；再次 publish 若指標被改過 → 409 `Publish Conflict`、不寫入，`PublishConflictDialog` 問作者、確認才 `?force=true` 覆寫並重設基準（workspace 與 CQL 預覽兩個入口都接）。V76 之前發布的沒有基準、不擋（漂移顯示為未知）。**不做**：從指標反推回 builder 樹（CQL → 視覺化樹不可逆）。**驗證**：後端 +10 案（`PublishedContentTest` 3、`EcqmPublishServiceTest` 4、`EcqmPublishRoundTripIntegrationTest` 1（真實 JPA flush + clear 往返）、controller 2；既有 publish controller 測試改 3 參數）、前端 +7 案（`BuilderSourceBanner` 3、`EcqmCqlPreviewTab` 衝突流程 2、`EcqmPage` deep link 2）。 | 需求 #939（B 級）、設計 #940、風險 #941、驗證 #942；**V76 migration**；可見變更：新端點 `GET /measures/{id}/builder-source`、publish 可能回 409、`EcqmArtifact.publishedAt` |[`9fdce40`](../../commit/9fdce40) |
| PAT-237 | ✨ feat | 2026-09-24 | 後端（`model/CqlTranslationResponse`（`ExpressionInfo.kind` / `operands`、`OperandInfo`）、`service/cql/CqlTranslationService`（`expressionInfo` / `typeName`）、`controller/CqlController`（`GET /libraries/{id}/expressions`）、`service/authoring/ExpressionCqlEngine`（`externalCqlFunctionCall`：`emitFunctionCall` / `resolveFunctionArgument` / `functionLiteral` / `functionCallQualifier` + include）、`TemplateService` 白名單）、前端（新增 `utils/libraryFunctions.ts`、`contexts/ArtifactScopeContext`、`components/authoring/builder/FunctionArgumentsEditor`；`ArtifactElementBody`、`ArtifactWorkspace`、`element-select/ElementSelect(Dropdown)`、`cql-libraries/LibraryDefinitionPicker`、`utils/libraryReference`、`ecqm/EcqmArtifactWorkspace` / `EcqmPopulationTreeEditor`、`external-cql/ExternalCql`、`api/cqlApi`、`hooks/useCqlLibraries`、`types`、`locales/*/{authoring,cqlLibraries}.json`）、smoke scenario 39 + `assert-measure-package.sh` 的 `package.cqlContains`、`docs/API.md`、`CLAUDE.md` ×3 | **Builder 呼叫程式庫函式（含引數）** — eCQM 缺口第 7 項的第一步：院內把共用邏輯寫成 `define function`，builder 作者在樹裡選函式、逐一填引數，產生 `"Lib"."Fn"(arg, …)`。**metadata 認得函式**：ELM 掃描以 `instanceof FunctionDef` 設 `kind` 與 operand 簽章（型別以作者寫法呈現：`Integer` / `List<FHIR.Observation>` / `Interval<DateTime>` / `Tuple{…}`；5.x 的 `Element` 只有 `getResultTypeName` / `getResultTypeSpecifier`）；新端點 `GET /api/cql/libraries/{id}/expressions` 給共用程式庫。**新元素 `externalCqlFunctionCall`**：來源欄位 static、`arguments[]` 每個引數 `mode` = 基礎元素 / 參數 / 字面值（Integer / Decimal / String / Boolean / Date / DateTime / Quantity，每型一種白名單形狀，String 跳脫）/ Patient / Measurement Period（只有 eCQM）；**任一引數解不開 → 整個呼叫 `null` + 警告**（少一個引數是另一個呼叫）；include 的 `called` 與呼叫用同一個消毒後的限定詞。**前端**：`libraryFunctions.ts`（元素工廠、預設引數模式、與後端一對一的驗證）、`ArtifactScopeContext`（CDS / eCQM workspace 各提供基礎元素、參數、是否有 Measurement Period）、`FunctionArgumentsEditor`（元素卡片上的 `arguments` 欄位，即時錯誤）；CDS palette 多「External CQL Functions」；共用程式庫 picker 改吃 ELM metadata、列函式簽章、確認時帶 `kind` / `operands`，`libraryReferenceToElement` 對函式回呼叫元素（eCQM 母群樹亦然）。**順修既有缺陷**：函式以前被當普通 define 列出、選了產生沒括號的 `"Lib"."Fn"` 到 publish 才爆；apply-library 把函式塞成 baseElement、modifier 合成把任何 define 當一元函式（現只收一元函式；無 `kind` 的舊 metadata 維持舊行為）。**驗證**：後端 +14 案（metadata 簽章 1、engine 6、eCQM builder 1、CDS 持久化 1、controller 2、驗證器 BUG-146 2 + 白名單）、前端 +15 案（`libraryFunctions` 5、`FunctionArgumentsEditor` 5、picker 1、`libraryReference` 2 …）；smoke 新 scenario 39（五種引數來源、4 / 3 / 2 → 66.67、封裝 round trip、`cqlContains` 鎖呼叫文字）。 | 需求 #934（B 級）、設計 #935、風險 #936、驗證 #937；無 migration；可見變更：`ExpressionInfo` 多 `kind` / `operands`、新元素型別、新端點；PAT-237 之前上傳的外部程式庫要重新上傳才有 `kind` |[`93012db`](../../commit/93012db) |
| BUG-146 | 🐛 fix | 2026-09-24 | `service/ecqm/EcqmExpressionTreeValidator`（`validateParameters`）+ 測試 | **eCQM artifact 帶型別參數存不進去** — eCQM 樹狀驗證器把 `parameters[]` 當元素樹走訪，`type: integer` / `decimal` / `string` / … 被當成未知元素型別，任何帶參數的 eCQM artifact 存檔一律 400 `unknown element type 'integer'`（CDS 驗證器從來不這樣做；做 PAT-237 的 smoke 才踩到）。改為參數專用檢查：名稱嚴格 HTML 檢查、comment 與字串預設值走 `@NoXss` 模式（`threshold < 7%` 是合法文字）、型別白名單 = `mapParameterType` 的鍵。 | 隨 PAT-237 同一 PR；驗證 #937 |[`93012db`](../../commit/93012db) |
| PAT-236 | ✨ feat | 2026-09-24 | 後端（`model/measure/MeasureDefinition`（8 個標準欄位 + `DefinitionTerm`）、`entity/MeasureDefinitionEntity` / `EcqmArtifactEntity`、`model/ecqm/EcqmArtifactRequest` / `EcqmArtifactResponse`、`service/measure/MeasureDefinitionService` / `CqfmMeasureBuilder` / `FhirMeasureService` / `HumanReadableService`（三個新段落 + TOC，id `standard-metadata` 不與既有 `metadata` 段衝突）、新增 `MeasureMetadataRules`、`service/ecqm/EcqmArtifactService` / `EcqmPublishService` / `EcqmExpressionTreeValidator`、`model/fhir/FhirCodeSystemConstants`、V75 + rollback）、前端（新增 `components/measure/MeasureStandardMetadataFields` + `utils/measureMetadata.ts`、`MeasureDetailsTab`、`EcqmSummaryTab`、`types` / `constants` / `locales/*/{measures,ecqm}.json`）、smoke scenario 33 + `assert-measure-package.sh` 的 `package.standardMetadata`、`docs/API.md`、`CLAUDE.md` ×3 | **指標標準 metadata：measure type、名詞定義、臨床建議聲明、生效 / 核准 / 審閱日期、experimental** — 指標與 eCQM artifact 都多 `measureTypes[]`（FHIR measure-type：process / outcome / structure / patient-reported-outcome / composite，最多 5）、`definitionTerms[] {term, definition}`（最多 50）、`clinicalRecommendationStatement`、`effectiveStart` / `effectiveEnd` / `approvalDate` / `lastReviewDate`、`experimental`（**V75**：`measure_definition` 與 `ecqm_artifact` 各 8 個 nullable 欄位）。**匯出**：CQFM `Measure.type[].coding`、`definition[]`（markdown `**term**: definition`）、`clinicalRecommendationStatement`、`effectivePeriod`（有起日才取代建立日期的舊 fallback）、`approvalDate` / `lastReviewDate` / `experimental`——只寫作者填的，匯入基底的同名元素只補空缺；**匯入**反向讀回（`definitionTerm` 解 `**term**: …`，其他工具的 markdown 整段當定義）；human-readable 多「Measure Metadata / Clinical Recommendation Statement / Definitions」三段。**publish** 只在 artifact 有值時覆蓋指標，重新發布不清掉指標頁填的內容。**前端**：指標編輯頁新增「標準 Metadata（FHIR Measure）」手風琴（類型 chip 摘要、experimental 警示 chip）、eCQM 摘要分頁同一組欄位——共用 `MeasureStandardMetadataFields`（類型複選 chip、experimental switch、四個日期、聲明字數、名詞列表增刪）；生效迄日早於起日即時標錯並鎖住儲存，後端兩個 service 也以 `MeasureMetadataRules` 擋 400。**artifact PUT 是部分更新**：四個日期在 request 是 ISO 字串，`""` = 清除、缺鍵 = 保留（`clearedDatesAsEmpty` 把元件的 `null` 換成 `""`）——先試過 `Optional<LocalDate>`，經 HTTP converter 缺鍵也會變 `Optional.empty()` 分不出來，controller 測試鎖住這個契約。artifact 的 `definitionTerms` 走 `@NoXss` 同一套 prose 規則而不是樹狀驗證器的「不准有角括號」（`HbA1c < 7%` 是合法定義）。**驗證**：後端 +20 案（CQFM 匯出 / 往返 / 基底優先序 / markdown 解析 6、publish 覆蓋規則 1、measure service 往返 + 倒置期間 2、artifact service 部分更新 / 倒置 / 假日期 / 建立複製 4、controller 契約 2、human-readable 3、樹狀驗證器 prose 2），前端新增 `MeasureStandardMetadataFields.test.tsx` 8 案 + `EcqmSummaryTab.test.tsx` 2 案；smoke scenario 33 的 artifact 帶齊全部欄位，`assert-measure-package.sh` 在匯出 Measure 與匯入結果兩端各檢查一次（V75 套用）。 | 需求 #929（B 級）、設計 #930、風險 #931、驗證 #932；**V75 migration**；疊在 PR 928（PAT-235）之上；可見變更：`MeasureDefinition` / `EcqmArtifact` 多 8 欄、CQFM Measure 多 `type` / `definition` / `clinicalRecommendationStatement` / `approvalDate` / `lastReviewDate` / `experimental` / `effectivePeriod.end` |[`b1e3a66`](../../commit/b1e3a66) |
| PAT-235 | ✨ feat | 2026-09-24 | 後端（`model/measure/StratifierDefinition`（`components[]`）、`MeasureEvaluationResult`（`StratumComponent`）、`service/measure/StratifierEvaluator` / `ValueKeys` / `MeasureValidationService` / `MeasureReportNormalizer` / `NormalizedMeasureReportReader` / `MeasureReportExportService` / `CqfmMeasureBuilder` / `FhirMeasureService` / `HumanReadableService`、`service/ecqm/EcqmCqlBuilder` / `EcqmPublishService`、`entity/MeasureReportStratifierEntity`、V74 + rollback）、前端（`components/ecqm/EcqmStratifiersTab`、新增 `utils/stratifierComponents.ts`、`EvaluationResultCard` / `PopulationCriteriaTab` / `TestCaseExpectedValuesEditor` / `types` / `locales/*/{ecqm,measures}.json`）、smoke scenario 38 + `assert.sh` 的 `components`、`docs/API.md`、`CLAUDE.md` ×3 | **多元件分層（FHIR `stratifier.component[]`）：性別 × 年齡帶一次宣告，報表一張交叉表** — 分層第三種模式「多元件」：每個元件有代碼與自己的條件式 / 值型運算式（重用 `ValueSourceEditor`），病人的 stratum 是各元件值的組合（QM IG 3.17）。**CQL**：每元件一個 define `Stratifier <id> <code>`（group 層級加後綴）；元件代碼白名單（英數 / 空格 / `_.-`、1–50 字、不重複），任一元件不合法 → **整個分層不產生**並警告（少一個元件就是另一個分層，寧可不出）。**評估**：不改 `MeasureEvaluationService` 傳遞的累積結構，多元件 key 編成自描述字串（`code` 0x1E `value` 0x1F …），`ValueKeys.of` 剝掉這兩個控制字元使值偽造不了元件邊界；`buildStratifierResults` 解回 `StratifierResult.components[] {code, value}`、`strataValue` 顯示為 `female \| 65+`；**任一元件無值 → 不屬任何層**（與單一分層的 null 規則一致）；`resolveStratumValue`（PAT-228 測試案例）回同一個顯示字串，期望值格式不變。**持久化 V74**：`measure_report_stratifier.component_values TEXT`（JSON，單一分層 NULL），normalizer 寫、reader 讀（壞 JSON 隔離）。**匯出 / 匯入**：MeasureReport `stratum.component[] {code, value}`（`value.text` 保留組合）；CQFM `stratifier.component[] {id, code, description?, criteria}`、無 stratifier 層級 criteria，匯入讀回、變更指紋含元件；human-readable 列 `code: define × …`；驗證服務對每個元件檢查 define。**前端**：三段切換（條件式 / 值 / 多元件），多元件起手為性別 × 年齡帶，元件卡（代碼、類型、編輯器、移除）+ 即時驗證；結果卡片每元件一個 chip；指標編輯頁對多元件分層只顯示；期望值編輯器不建議 true / false。單一分層行為、資料、匯出、smoke 字串比對**一字不改**。**驗證**：後端 1944 案（+8：評估器 2、builder 1、publish 1、normalizer 1、reader 1、export 1、CQFM 往返 1）、前端 908 案（+5）、smoke 38/38（新 scenario 38：性別 × 年齡帶五個交叉層的計數 / 分數 / `components` / `exactStrata`——男 50-64 沒人就沒有層；V74 套用）。 | 需求 #924（B 級）、設計 #925、風險 #926、驗證 #927；**V74 migration**（加一個 nullable 欄位）；可見變更：`StratifierResult.components`、MeasureReport `stratum.component`、CQFM `stratifier.component` |[`31cb4ba`](../../commit/31cb4ba) [`b1e3a66`](../../commit/b1e3a66) |
| PAT-234 | ✨ feat | 2026-09-23 | 後端（新增 `service/measure/ValueKeys` / `SupplementalDataEvaluator`、`entity/MeasureReportSupplementalDataEntity`、`repository/MeasureReportSupplementalDataRepository`、V73 + rollback；`MeasureEvaluationResult` / `MeasureEvaluationService` / `StratifierEvaluator` / `EcqmCqlBuilder` / `EcqmPublishService` / `MeasureReportNormalizer` / `NormalizedMeasureReportReader` / `MeasureReportExportService`）、前端（新增 `components/ecqm/ValueSourceEditor`；`EcqmSdeTab` / `EcqmStratifiersTab` / `EvaluationResultCard` / `types` / `locales/*/{ecqm,measures}.json`）、smoke scenario 37 + `assert.sh` 的 `supplementalDataResults`、`docs/API.md`、`CLAUDE.md` ×3 | **風險校正進 builder，補充資料的值真的出現在報表** — eCQM workspace 的 SDE 元素多「用途」（`supplemental-data` / `risk-adjustment-factor`，QM IG 3.19）與「類型」（布林條件樹 / 值型，值型共用 PAT-233 的性別 / 年齡帶來源，編輯器抽成 `ValueSourceEditor`）；切成 RAF 時預設名稱自動改成 `RAF …`，自取名字只提示、builder 產生 CQL 時警告。**publish 依 usage 映進 `MeasureDefinition.supplementalData` / `riskAdjustments`**——以前一個都沒映，評估與交換封裝（PAT-229 已會依 usage 輸出）都不知道指標有 SDE。**評估改為值分布**：新 `SupplementalDataEvaluator` 對指標宣告的每個 SDE / RAF define 逐病人以 `ValueKeys.of`（從 `StratifierEvaluator.stratumKey` 抽出、分層與 SDE 共用的同一條 key 規則）分桶成「值 → 病人數」，無值病人另計，宣告順序 / 首次出現順序輸出；以前 `aggregateCustomExpressions` 把非母群 define 縮成一個計數、**字串直接丟掉**（`SDE Sex` 在報表裡等於不存在），舊計數 map 保留但 UI 有分布時不再顯示。**持久化**：V73 `measure_report_supplemental_data`（一列一值、null 值列 = 無值病人數、usage CHECK、FK cascade、RLS + FORCE 經 `measure_report.tenant_id`，與 V70 的子表寫法一致），normalizer 先刪後寫、reader 重建（以前 reader 對 supplementalData 一律留 null）。**匯出**：FHIR MeasureReport 以平台 extension `<canonical base>/StructureDefinition/measurereport-supplemental-data` 攜帶（R4 summary 報表沒有標準元素放 SDE 分布，標準把 SDE 放在 individual 報表逐病人；不捏造 DEQM URL），CSV 段落 + Excel「Supplemental Data」工作表。**UI**：評估結果卡片新表（定義 / 用途 chip / 值 / 病人數 / 占比 / 無值列）。**這不是風險模型**：IG 只帶變數，校正後的率需要指標之外的係數。**驗證**：後端 1936 案（+8：`SupplementalDataEvaluatorTest` 3、publish 1、builder 1、normalizer 1、reader 1、export 1）、前端 903 案（+5）、smoke 37/37（新 scenario 37：標準 `SDE Sex` `{female 4, male 3}`、值型 RAF 年齡帶 `{18-49 2, 50-64 2, 65+ 3}`、布林 RAF `{true 3, false 4}`，V73 在 app role + strict RLS 下寫入）。 | 需求 #919（B 級）、設計 #920、風險 #921、驗證 #922；**V73 migration**（新表）；疊在 PAT-233（PR 918）之上，合併順序 918 → 本 PR；可見變更：`MeasureEvaluationResult.supplementalDataResults`、MeasureReport 匯出多 extension、CSV / Excel 多段落 |[`e90ca0d`](../../commit/e90ca0d) |
| PAT-233 | ✨ feat | 2026-09-23 | 後端（`model/measure/StratifierDefinition`、`service/ecqm/EcqmCqlBuilder` / `EcqmPublishService`、`service/measure/StratifierEvaluator` / `MeasureEvaluationService` / `MeasureReportExportService`、`service/cql/CqlValues` / `CqlExecutionService`）、前端（`components/ecqm/EcqmStratifiersTab`、新增 `utils/ageBands.ts`、`types/ecqm.ts` / `types/index.ts`、`constants/ecqmConstants`、`TestCaseExpectedValuesEditor`、`locales/*/ecqm.json`）、smoke scenario 36 + `lib/assert.sh` 的 `exactStrata`、`CLAUDE.md` ×2 | **值型分層（value stratifier）：builder 以性別 / 年齡帶分層，值就是 stratum** — eCQM workspace 的分層多「類型」：條件式（原本的布林樹 → `true` / `false`）或**值**（QM IG 3.17：運算式的結果就是 stratum，每個不同的值一層）。值型只有結構化來源——`gender` → `Patient.gender.value`；`ageBands` → `case when AgeInYearsAt(end of "Measurement Period") >= min and … <= max then '<label>' … else null end`（上下界含、整數歲 0–150、標籤限 ASCII `[A-Za-z0-9 _+-./:()]`、不重複、≤ 20 帶，前後端同一套檢查，不合法者不產生 define 並警告）；**不做自由 CQL 欄位**（artifact 是 client JSON，那是 injection 面）。評估層本來就以 `String.valueOf(value)` 當 stratum key，收成單一規則 `StratifierEvaluator.stratumKey`（Code map → `code (display)`、Concept → display、清單相接、空 / `null` → 不屬任何層、200 字截斷），分層依首次出現順序。**順手修掉三個讀代碼確認的既有缺陷**：(1) eCQM workspace「Stratifiers」分頁編輯的是 artifact 層級 `stratifiers`，publish 只映 group 層級（UI 又不編那層）→ builder 做的分層有 CQL define 卻從不進報表；現在套到每個 group（無後綴 `Stratifier <id>`，帶 `kind` / `description`）。(2) 分層分數一律 proportion 公式 → 改依 scoring type（ratio 不扣 exclusion、cohort 給該層 IP、CV 不給分數——observation 值沒有逐層收集）。(3) FHIR MeasureReport 匯出每個 stratum 各寫成一個沒有 `code` 的 `stratifier` → 改為每分層一筆 `{code.text, stratum[]}`，population coding 補 system。**值的序列化**：`CqlValues.primitiveValue` 以形狀（只有 `value` 元素，另可 `id` / `extension`）判斷 FHIR primitive → `toSerializable` 回其值（`Patient.gender` 是 `female` 而非 `FHIR.AdministrativeGender`——第一版用型別名首字小寫判斷，真引擎立刻打臉）；`Code` → `{code, system, display, version}`、`Concept` → `{codes, display}`；`Quantity` / `Identifier` 等有其他元素者不受影響。前端：類型切換、來源選單、年齡帶編輯（預設 0-17 / 18-49 / 50-64 / 65+ 起手、標籤跟著上下界、即時 Alert）、測試案例期望值編輯器對值型分層不再建議 true / false。**驗證**：後端 1928 案（+14：builder 4、publish 1、`StratifierEvaluatorTest` 7、`MeasureReportExportServiceTest` 1、真引擎整合 1）、前端 898 案（+7）、smoke 36/36（新 scenario 36：三個分層 gender / sex / age 的每層計數與分數、`exactStrata`；19 / 27 期望值未動）。 | 需求 #914（B 級）、設計 #915、風險 #916、驗證 #917；無 migration；可見變更：MeasureReport 匯出的 stratifier 形狀、執行結果中 primitive / Code 的呈現、`group_definitions` 多 `kind` |[`d86f955`](../../commit/d86f955) [`e90ca0d`](../../commit/e90ca0d) |
| PAT-232 | ✨ feat | 2026-09-22 | 後端（新增 `service/cql/ClauseCoverageCollector`、`model/measure/ClauseCoverage` / `MeasureClauseCoverage`；`CqlExecutionRequest` / `CqlExecutionResponse` / `CqlExecutionService` / `TestCaseService` / `TestCaseRunResult` / `MeasureController`）、前端（新增 `utils/clauseCoverage.ts`、`components/measure/ClauseCoverageView.tsx`；`TestCasesTab` / `measureApi` / `types` / `locales/*/measures.json`）、smoke scenario 35 + `lib/assert-clause-coverage.sh`、`docs/API.md`、`CLAUDE.md` | **逐子句測試覆蓋率（Bonnie / MADiE 式）** — 測試案例以除錯模式執行時，回應多出 `clauseCoverage`：CQL 全文、每個 define / function 的每個子句（ELM 有 `localId` + `locator` 的運算式節點）的位置、型別、執行次數與最後一次的值，以及已覆蓋 / 全部子句數與百分比；新端點 `POST /measures/{id}/test-cases/coverage` 執行全部測試案例並回傳**聯集**（任一案例執行到即算覆蓋、`hits` 相加、只合併同一份 CQL 文字）與執行 / 通過數。**做法**：分母靜態（`BaseElmLibraryVisitor` 走訪翻譯結果）、分子動態（cql-engine 5.x 的 `BreakpointHandler.onAfterExpression`，掛在 `engine.getState()`），兩者對同一次翻譯的 `Library`，所以「未執行」是引擎事實——沒有病人資料時 query 的 `where` 與 observation 計算式從未迭代、引擎不對 `and` 短路（兩側都執行）、query 逐次迭代各記一次，都以真引擎測試鎖住而非語言假設。handler 對每個運算式節點各呼叫一次，**只在** `CqlExecutionRequest.clauseCoverage=true`（測試案例除錯模式與指標整體覆蓋率）時建立，`$evaluate-measure` 與一般測試案例執行不付成本、回應也沒有此欄位；覆蓋率不持久化。前端 `ClauseCoverageView` 以 locator 把 CQL 原文切段（內層子句優先、1-based 結尾含、CRLF 只以 `\n` 計行），覆蓋 / 未覆蓋以底色與底線標示、tooltip 顯示 `型別 → 值 (n×)`、依敘述列 chip、可只看有缺口的敘述；每筆除錯結果與指標整體各一個 Accordion。子句值對 FHIR 資源只給 `FHIR.Type/id`、80 字元截斷。**smoke 在真實產生的 eCQM CQL 上抓到、單元測試沒抓到的兩個分母錯誤**：翻譯器在父節點之間共用節點（`duration in days of E.period` 的 `E.period` 同時在 `Start` / `End` 下），單純走訪把同一子句列兩次、`hits` 也算兩次（單次執行分母 13、聯集 11）；`context Patient` 的隱式 `define "Patient": SingletonFrom([Patient])` 進了分母但引擎不經 visitor 評估它，有資料的病人永遠到不了 100%——`universe` 改以 `localId` 去重並略過隱式 context 定義。**驗證**：後端新增 13 案（`ClauseCoverageCollectorTest` 9 案打真引擎：靜態枚舉與動態記錄一致、唯一未覆蓋的是 `else` 分支、逐次迭代、函式為獨立敘述、retrieve 值、merge 聯集、旗標關閉不收集、值截斷、eCQM 產生器同形狀的共用節點只算一次且每案斷言 `localId` 唯一；`TestCaseClauseCoverageTest` 4 案）；前端新增 12 案；smoke 新增 scenario 35（有資料病人 100%、無資料病人有未覆蓋子句、一般執行沒有欄位、聯集同一份 CQL 同一個子句總數、`hits` 相加）。 | 需求 #909（B 級）、設計 #910、風險 #911、驗證 #912；疊在 PAT-231（PR 908）之上，需先合併；無 migration |[`9444cdf`](../../commit/9444cdf) |
| PAT-231 | ✨ patch | 2026-09-22 | 後端（`pom.xml` cql 4.8.0 → 5.3.0、`engine-fhir` → `engine-fhir-jvm`；新增 `service/cql/CqlValues`；`CqlExecutionService`、5 個 RetrieveProvider、`ComparableR4FhirModelResolver`、`FhirDataProviderService`）、`CLAUDE.md` / `backend/CLAUDE.md` | **CQL 引擎升版 4.8.0 → 5.3.0（runtime 值模型遷移）** — 5.x 把引擎的所有值從 `Object` 改成 `runtime.Value` 介面（`runtime.Boolean` 等基本型別、`ClassInstance` 樹取代 HAPI 物件）、`RetrieveProvider.retrieve` 回傳 `Iterable<Value>`、`ModelResolver` 移除 `resolvePath(Object,String)` / `resolveType(Object)`。Dependabot #881 紅的第一關是 artifact 改名（5.x 的 `engine-fhir` 是 0 class 空殼，內容在 `engine-fhir-jvm`）。**做法**：新增 `CqlValues` 作為 4.x 與 5.x 之間唯一的橋接（`fromFhir` / `wrap` / `unwrap` / `describe`）；`CqlExecutionService` 四個取值點經 `unwrap` 轉回純 Java——這是最危險的一層：`Object v = result.getValue()` 仍編得過但 `instanceof Boolean` 變 false，母群會靜默清空；5 個 RetrieveProvider 改 `Iterable<Value>`，`PrefetchRetrieveProvider` 與 fallback search 產生的 HAPI 資源經 `fromFhir`；`ComparableR4FhirModelResolver` 的兩個 override（Encounter.class、BUG-106）刪除——5.x 不再反射 HAPI 物件而是用 HAPI runtime definition 轉成 `ClassInstance`，兩個問題由設計解掉，`CqlValuesTest` 直接鎖定。**讀碼發現、測試抓不到的退化**：`unwrap` 把 `Tuple` 變成 `Map`，原 `toSerializable` 只認 `runtime.Tuple`，Map 會落到字串分支使 CDS tuple card 拿到文字；補 Map 分支並以整合測試鎖住回應形狀。FHIR 資源的顯示改為 `FHIR.Type/id`（4.x 為 `Type@hash`；`ClassInstance.toString()` 是整棵樹）。**CI Security Scan 抓到的上游打包退化**：`quick:5.3.0` 把相依從 `jaxb2-basics-runtime` 改成整個 `jaxb2-basics` XJC 外掛，順帶拖進建置期的 `jaxb2-basics-tools` 與 `commons-beanutils 1.9.3`（CVE-2014-0114 / CVE-2019-10086 / CVE-2025-48734，3 個 HIGH）；runtime 只需要 `-runtime`，於 pom 排除 `-tools`。**驗證**：後端完整套件 1893 → 1902 案（新增 9）全過、smoke 34/34、backend log 的 ERROR/WARN 模式與 4.8 完全相同。這是升到 5.x 的前提工作：逐子句 coverage 需要 5.x 的 `BreakpointHandler`。 | 需求 #904（B 級）、設計 #905、風險 #906、驗證 #907；無 migration、無 API 變更；取代 dependabot PR 881 |[`ec3afbc`](../../commit/ec3afbc) [`9444cdf`](../../commit/9444cdf) |
| BUG-145 | 🐛 fix | 2026-09-21 | 後端（新增 `util/Stopwatch`；`MeasureEvaluationService`、`MeasureReportService.saveReport`）、smoke harness（`scripts/smoke/run.sh` 報表儲存檢查） | **評估耗時為負值時，評估報表存不進去而且沒有任何跡象** — `MeasureEvaluationService` 以 `System.currentTimeMillis() - startTime`（牆上時鐘相減）計算耗時；`measure_report.evaluation_duration_ms` 自 V51 起有 CHECK `>= 0`。系統時鐘在評估期間被往回校正時耗時為負，insert 被 `chk_measure_report_duration_nonneg` 拒絕；`autoSaveReport` 刻意不讓儲存失敗影響評估回應，於是 API 回 200、數字正確、報表遺失，只有 log 留下一行 ERROR。2026-09-20 的 smoke 執行實際發生 3 筆（同一個 3 毫秒內、並行評估的 scenario）；觸發原因**推測**為 Docker Desktop VM 的時鐘校正（未驗證），缺陷本身由程式碼與 log 即可確認。**修正**：(1) 耗時改用單調時鐘——新增 `util/Stopwatch`（`System.nanoTime()`），取代 `evaluateMeasure` 內三處牆上時鐘相減；(2) `saveReport` 收到負的耗時時存 NULL（constraint 允許 NULL 表示未知；不存 0，那會謊稱評估沒花時間）並記錄警告，報表照常儲存——附屬資訊不得拖垮紀錄；(3) smoke harness 新增報表儲存檢查：後端 log 出現報表儲存失敗即判定整個執行失敗並印出 constraint 名稱，以當次實際含 3 筆失敗的 log 驗證過判斷式。修正前 `MeasureReportServiceDurationTest` 為紅（`-37` 被原樣寫入）。同類的牆上時鐘相減還散見於其他 12 個檔案（多為 log / 回應中的耗時；會儲存的另一處是 CDS 分析的累加回應時間，無 CHECK、不會造成紀錄遺失），不在本次範圍。 | 需求 #899（B 級）、設計 #900、風險 #901、驗證 #902；無 migration、無 API 變更 |[`e467103`](../../commit/e467103) |
| BUG-144 | 🐛 fix | 2026-09-20 | 後端（`config/DataInitializer`）、smoke harness（`scripts/smoke/run.sh` 開機檢查） | **空資料庫第一次啟動會崩潰一次，且示範指標永遠不會建立** — `DataInitializer.seedDemoMeasure()` 建立示範指標時沒有設定 `tenantId`，而 `measure_definition.tenant_id` 自 V61 起為 NOT NULL：insert 失敗 → `Application run failed` → 容器依 `restart: unless-stopped` 重啟 → 此時 `admin` / `demo` 兩個帳號已存在，`userRepository.count() == 0` 不成立，整段初始化被跳過。結果是全新安裝第一次開機崩一次、示範指標不會被建立；沒有自動重啟的環境則直接起不來。實際觀察到的是 2026-09-19 與 09-20 的 smoke 執行；依程式碼推論，自 V61（2026-07-09）把欄位改為 NOT NULL 起每個空資料庫都會如此（未回頭驗證舊版）。對既有正式環境無影響（已有使用者，初始化不執行）。**為何沒被發現**：`DataInitializer` 只在 `dev` / `docker` profile 執行，單元與整合測試用 `test` profile + H2（Flyway 關閉）；smoke harness 只等 health 變綠，看不到中間那次重啟。2026-09-19 做 PAT-229 時由 smoke 的 backend log 觀察到。**修正**：示範指標明確寫入 `default` 租戶；沒有 `default` 租戶時略過並警告；示範資料的任何 runtime 例外只記錄錯誤、不中斷啟動（兩個帳號仍建立）。**防止再犯**：smoke harness 新增開機檢查——後端容器重啟次數不為 0 即失敗並印出 `Caused by`，並確認示範指標存在；修正前以同一檢查實測為紅（1 次重啟）。已崩過一次的既有安裝不回溯補建示範指標（只在第一次啟動種資料是原設計，事後自動新增公開指標不恰當）。 | 需求 #895（A 級）、驗證 #896；無 migration |[`c7dc176`](../../commit/c7dc176) |
| PAT-230 | ✨ feat | 2026-09-19 | 後端（`V72` `value_set` + rollback、`ValueSetEntity` / `ValueSetRepository`、`PlatformValueSetService`、`PlatformTerminologyProvider`、`ValueSetController`；`FhirTerminologyService`、`FhirController`、`ExpressionCqlEngine` / `CqlArtifactBuilder` / `EcqmCqlBuilder` + 兩個主模板、`FhirMeasureBundleService` / `FhirMeasureBundleImportService`）、前端（術語頁「本單位 value set」分頁、`valueSetApi`、`usePlatformValueSets`、`utils/valueSetCodes`、搜尋結果來源標示）、smoke scenario 34、`docs/API.md` §5.4 | **平台自有 value set：建立、版本與釘選、引擎解析、FHIR 匯入匯出、隨指標封裝交換** — eCQM authoring 完整度盤點第 5 項（術語只能查不能建）。原本 value set 只能由內建 TW Core IG、VSAC、公開術語伺服器查詢，醫院自訂的代碼清單（院內 / 健保醫令、處方集、診斷分組）無處存放；台灣沒有 VSAC 的對應物。**本次**：(1) `value_set` 資料表（租戶範圍；一列 = 一個版本；`(tenant_id, url, version)` 唯一；內容為明確代碼清單）。(2) 生命週期 draft → active → retired：**啟用即凍結**，改代碼只能建立新版本；CQL `version '…'` 釘選時解析該版本，未釘選解析最新 active，尚無 active 時解析最新 draft（讓作者啟用前可測試），retired 只在釘選時解析；空的 value set 不可啟用；已儲存的代碼讀不回來時明確失敗，不當成空清單。(3) 引擎：`PlatformTerminologyProvider` 排在 IG / VSAC / 術語伺服器之前。租戶在 provider 建立時固定（底下的共用 provider 以伺服器 URL 為 key 全程序快取，引擎也可能從沒有租戶資訊的執行緒回呼）；每個引用每次評估只查一次，修改於下一次評估立即生效。(4) `$expand`、value set 搜尋、`$validate-code` 在 controller 層先查平台——不放進 `FhirTerminologyService`，因為它的 Caffeine 快取以 URL 為 key、跨租戶共用。(5) FHIR ValueSet 匯入（規則式定義需帶展開結果，否則拒絕並說明）／匯出（未修改的匯入資源原樣輸出）；交換封裝內嵌平台 value set 的完整代碼，匯入封裝時把 value set 存為本租戶的 draft（已存在者保留本機內容並警告）——之前只是數一數就丟掉。(6) 術語頁管理畫面：清單、建立 / 編輯草稿、由試算表或 CSV 貼上代碼（回報讀不到的行號）、啟用 / 停用 / 新版本 / 刪除草稿、複製 CQL 宣告（可釘選）、FHIR 匯入匯出；builder 與術語頁的搜尋結果標示「本單位」來源。**一併修正的既有缺陷**：(a) **builder 產生的 CQL 丟掉了 value set 的 URL**——挑選器存 `{name, oid}`，產生器只讀 `name`，標頭宣告成 `valueset "<名稱>": '<名稱>'`，沒有任何術語來源能解析；沒有任何測試或 smoke scenario 走過 builder 挑選的 value set。現在宣告作者選定的 URL（裸 OID 轉 `urn:oid:`、可附 `version`；沒有 URL 的舊資料維持原宣告）。(b) 代碼驗證分頁一律 400：前端送 `url`（FHIR 標準），後端要求 `valueSet`；現在兩者皆接受。 | 需求 #888（B 級）、設計 #889、風險 #890、驗證 #891；migration V72（須在 V71 之後部署）；未納入 RLS（與 `cql_library` 相同，非 PHI） |[`00682d8`](../../commit/00682d8) |
| PAT-229 | ✨ feat | 2026-09-19 | 後端（新增 `CqfmConstants`、`CqfmMeasureBuilder`、`CqfmLibraryBuilder`、`FhirCanonicalResolver`、`MeasureExportConformance`、`Jackson2TreeBridgeConfig`；改寫 `FhirMeasureService` / `FhirMeasureBundleService` / `FhirMeasureBundleImportService`；`MeasureController`）、前端（`MeasureExportConformanceDialog` + 匯出選單）、`FHIR_CANONICAL_BASE`、smoke scenario 33、`docs/API.md` §4.2 | **指標交換封裝符合 HL7 Quality Measure IG / CRMI，匯入後可評估** — eCQM authoring 完整度盤點第 2 項。原本的 FHIR 匯出無法用於跨院交換：`Measure.library` 是相對參照、Bundle 內 ValueSet 的 `url` 填的是 CQL value set 的名稱、`dataRequirement` 被放在 Measure 上（R4 Measure 無此元素）、`in-review` 被當成 FHIR status、XML 匯出只有 5 個欄位、匯入的指標沒有 CQL 所以無法評估、`GET /{id}/fhir` 沒經過讀取檢查。**本次**：(1) Measure 依 Quality Measure IG 5.0.0 輸出——絕對 canonical url、`library` 為主要 Library 的 url 加版本、`text/cql-identifier`、group / population id、`cqfm-populationBasis` / `cqfm-scoringUnit` / `cqfm-aggregateMethod` / `cqfm-criteriaReference`、SDE 與 risk adjustment 的 usage 代碼、contained `effective-data-requirements`；平台已知的 metadata 全部輸出。(2) **不捏造**：`meta.profile` 只在必要條件成立時宣告，缺 title / description / improvementNotation 時不宣告並列入報告；profile / extension URL 逐一由官方 StructureDefinition JSON 核對。(3) 主要 Library 帶 CQL + ELM，名稱版本取自 CQL 的 `library` 宣告；CQL `include` 的相依 Library 由 ELM 逐層找出一併封裝；value set 由 IG / VSAC 解析到的附完整定義，解析不到的以真實 URL 標示並回報。(4) 新端點 `GET /api/measures/{id}/export/conformance` 與匯出選單的「交換封裝檢查」：profile、缺漏、value set 封裝狀態、canonical base、是否可交換。(5) 匯入由 `Measure.library` 判定主要 Library（不依順序猜），其 CQL 成為指標邏輯；匯入一律 `draft`。(6) 未修改的匯入指標再匯出時保留原資源與未知擴充欄位，僅 status 以本機為準。(7) XML 改由 HAPI 轉出完整 FHIR XML。**Smoke 33 抓到的既有缺陷**：Spring Boot 4 的 HTTP 轉換器是 Jackson 3，而 FHIR 服務用 Jackson 2 樹——三個以 `JsonNode` 為 request body 的匯入端點（Measure、Bundle、Library）一律 500（於 Boot 4.1.0 實測；推測自 2026-05 升 Boot 4.0 起即如此，未回頭驗證舊版），兩個回傳 `ObjectNode` 的匯出端點回的是 `{"array":false,…,"nodeType":"OBJECT"}` 而非資源內容；此前沒有任何測試經過這五個端點的 HTTP 層。以 `Jackson2TreeBridgeConfig`（Jackson 3 module）修正，`FhirJsonBodyEndpointsTest` 7 案鎖定。Population basis 非 boolean 時報告會警告「本平台以病人為單位計數」；episode 層級計數不在本次範圍。 | 需求 #871（B 級）、設計 #884、風險 #885、驗證 #886；無 migration；新環境變數 `FHIR_CANONICAL_BASE`（選填） |[`78a6ee5`](../../commit/78a6ee5) [`00682d8`](../../commit/00682d8) |
| PAT-228 | ✨ feat | 2026-09-19 | 測試案例結構化期望值（`TestCaseExpectedValues`、`TestCaseService`、`PopulationEvaluator.evaluateSinglePatient`、`StratifierEvaluator.resolveStratumValue`、`V71`、前端 `TestCaseExpectedValuesEditor` / `TestCaseResult` / 匯入、smoke `32`） | **測試案例能逐 group 斷言母群生效歸屬、observation 數值與 stratifier 落點，且 actual 值一律由正式評估的同一套元件計算** — 原本期望值只有一張扁平的 `Map<String, Boolean>`：多 group 指標裡同名母群（兩個 group 都有 `numerator`）互相覆蓋；continuous-variable 與 ratio 指標真正要驗的 observation 數值完全無法斷言；stratifier 落點也不能測；而且執行時直接取每個 define 的原始真假值，被分母排除的病人其 `Numerator` define 常為 true，測試者填「分子：否」反而 fail。**(1) 模型**：新增 `expectedValues { groups: [{ groupId, populations, observations, stratifiers }] }`，期望與實際共用形狀；母群採整數計數（目前以病人為單位，0 或 1），為日後 episode 計數預留而不必改格式。**(2) 不重寫規則**：`PopulationEvaluator.evaluateSinglePatient` 對一張全 0 計數表呼叫既有的 `aggregatePatientResults` / `aggregateRatioPatientResults` / `aggregateCvPatientResults`，所以「測試通過」等同於「報表會以同樣方式計入這位病人」；逐 group 的 observation define 名稱規則抽成 `observationExpressionNames`、stratifier 分層值抽成 `resolveStratumValue`，`MeasureEvaluationService` 與 `StratifierEvaluator` 的彙總迴圈改走同一個方法（等價重構，行為不變）。**(3) 比對**：母群取期望與實際 key 的聯集（缺者視為 0）、observation 排序後以 1e-6 容差比對、stratifier 只比對有列出的；結果為 `valueComparisons`（groupId / kind / key / expected / actual / match）。有 `expectedValues` 時由它決定 pass / fail，沒有時走原本的布林比對（既有測試案例零變更），但每次成功執行都回報結構化 `actualValues`，編輯器可「帶入上次執行的實際值」。**(4) 不放過錯置的期望**：儲存時未知 group / 母群 / stratifier、負數、非數字 → 400 附 details；指標事後被改到 group 消失，執行結果為 fail 而非靜默通過。**(5) V71** `test_case.expected_values TEXT`（NULL = 沿用舊欄位）+ rollback；實體直接持有 JSON 字串，避開「只改 `@Transient` 物件不會被 dirty-check」的陷阱。**(6) 前端**：純函式模組 `utils/testCaseExpectedValues.ts`、元件 `TestCaseExpectedValuesEditor`（逐 group 的母群開關、observation 清單、stratifier 期望值）、`TestCaseEditor` 切換開關（新測試案例在指標有多 group / observation / stratifier 時預設開啟；舊模式下對這類指標顯示警示）、`TestCaseResult` 逐項比對表、匯入與草稿自動儲存帶上新欄位；i18n en / zh-TW。**開發過程由測試抓到的缺陷**：`isEmpty()` 被 Jackson 序列化成 `empty` 屬性，存進去的 JSON 讀不回來而靜默退回舊比對 —— 已加 `@JsonIgnore` 並以往返測試鎖住。**驗證**：後端 TestCaseExpectedValuesTest 16/16 + 完整套件 1809/0/0（7 略過）；前端新增 29 案、完整套件 852 全過、tsc / eslint / i18n 乾淨；smoke 32 的 14 項斷言全過、smoke 全套 32/32（RLS app role + strict、schema v71）。**重要發現（另案）**：正式評估的母群計數完全以病人為單位（`isPopulationTrue` 只看數量是否大於 0），builder 即使選 episode basis 也只有 CV 的 Measure Population 回傳資源清單；episode 層級的母群計數需變更正式評估器，屬獨立的 B 級變更，本次只在介面明示此限制、不擴張範圍。 | 需求 #866（B 級）/ 設計 #867 / 風險 #868 / 驗證 #869；V71 |[`3d9ca5e`](../../commit/3d9ca5e) |
| PAT-227 | 📝 docs | 2026-09-18 | 移除未引用的 `docker/Dockerfile.backend` / `docker/Dockerfile.frontend`；更正 PAT-224 列、DEPLOYMENT_GUIDE §6.1 建置階段、readiness review 參照 | **兩個死 Dockerfile 誤導了 PAT-224 的敘述** — compose、CI Docker Build、deploy.yml 一律以 `backend/Dockerfile`（`maven:3-eclipse-temurin-26` 建置、`eclipse-temurin:25-jre-alpine` 執行）與 `frontend/Dockerfile`（`node:26-alpine`）建置，`docker/Dockerfile.*` 沒有任何 compose / workflow / 腳本引用。appmod agent 在 PAT-224 改了 `docker/Dockerfile.backend`，CHANGE_LOG 與 PR 說明因此寫成「建置映像 temurin-25 / 執行映像 25-jre」，實際上正式映像在 PAT-224 之前就已跑在 25-jre（2026-09-18 部署勘查時由正式機 `java -version` 發現）。本次：刪除兩檔；PAT-224 列更正；§6.1 建置階段字樣改 `maven:3-eclipse-temurin-26`；readiness review 的「multi-stage + 非 root」證據改指 `backend/Dockerfile:19`。純文件與死檔清理，無建置或執行行為變更。 | 需求 #856 紀錄更正；無 migration |[`59750f5`](../../commit/59750f5) |
| PAT-168 | 📝 docs | 2026-05-28 | 文件（新增 `docs/integration/HIS_INTEGRATION.md`） | **HIS / EHR 對接指南 — FHIR-first 整合手冊** — 寫給三種讀者的整合手冊：(a) HIS 廠商工程師（需實作 endpoint 規格）、(b) 醫院 IT / 系統管理員（要在 CQL 後台設連線、跑健康監控）、(c) 內部開發者（要擴充對接支援）。**內容**：(1) 架構概覽明確標示 R4-only 限制（`FhirContext.forR4()` hardcoded）+ 為何選 FHIR-first 而非直 DB-to-DB；(2) 對接路徑決策樹（HIS 有 FHIR endpoint → 快通道，無 → 中介層方案）；(3) HIS endpoint 規格表：必/選 endpoints (`/metadata`、`Patient` search、`$everything`、5 種 fallback search、`Subscription`、`$export`)、Patient 搜尋必要 query 參數（`identifier`/`family`/`given`）、Patient 必要欄位（`identifier[*].value`、`name`、`birthDate`、`gender`）；(4) 認證 4 選 1 詳列：`none` / `basic` / `bearer` / `smart_backend`，其中 SMART Backend Services 完整列 OAuth Client Credentials + RS384 JWT assertion claims schema (`iss`/`sub`/`aud`/`jti`/`iat`/`exp`)；(5) Pull / Push / Bulk Export 三種流程；(6) TLS / mTLS 規格（PEM 格式、`tls_min_version` 預設 TLSv1.2、hostname verification 預設強制開啟 + sysadmin kill-switch `security.tls.allow-disable-hostname-verification`）；(7) TW Core IG 對齊（`tw.gov.mohw.twcore` v0.3.2、canonical `https://twcore.mohw.gov.tw/ig/twcore`、base FHIR 4.0.1、載入位置 `classpath:fhir-packages/twcore/package.tgz`）；(8) 醫院 IT 後台操作完整 cURL 範例（新增/測試連線/病人搜尋/preview/import/批次/Subscription/Health/Failed-Imports）+ 權限矩陣（ADMIN / DEPT_ADMIN）；(9) 中介層方案（HL7v2/CDA → FHIR R4 mapping 表 PID-3 / PID-5 / PID-7 / PID-8 / OBX / DG1 / RXE / PV1）；(10) 安全與合規（`EncryptionConverter` AES 自動加密 credentials + 三個 PEM 欄位、`APP_ENCRYPTION_SECRET` env 注入、`@JsonProperty(WRITE_ONLY)` 永不回吐、callback HMAC 風險警示、IEC 62304 法遵 hook）；(11) 故障排除表 — `lastTestMessage` 開頭對應的 6 種失敗模式（PKIX path building / hostname mismatch / 401 / 403 scope 不足 / Read timeout / SMART JWT 簽章失敗）+ 處理建議；(12) 開發者擴充指南：新增 auth type 5 步驟、擴充 FHIR R5 路徑說明 cql-engine 4.5.0 限制（有 R5 model resolver 但無 query generator / terminology provider）；(13) 附錄：29 個 REST endpoint 完整表、`EhrConnectionRequest` schema、credentials JSON schema（依 authType）、V28 migration table 摘要、6 個環境變數、明列**不在本次範圍**的項目（直 DB sync / HL7v2 內建解析 / R5 client / callback HMAC / 自動 token rotation）。**為何寫**：對接需求逐漸增加（多家 HIS 廠商詢問），原本資訊散落於 `EhrIntegrationController` / `FhirClientFactory` / `SmartBackendTokenService` / `TlsContextFactory` 註解與 `application.yml`，廠商與醫院 IT 入門成本高；統一文件降低初次對接溝通成本。**為什麼不寫 DB 直連章節**：平台目前**沒有** JDBC/CDC 直連 HIS DB 的 connector，寫了會誤導；明確標 §9.6「不在本次範圍」並指引若需 DB 同步應走 PAT-NNN 新需求 + TFDA 法規工作流。**驗證**：(a) recon 階段讀 `EhrIntegrationController.java`、`EhrConnectionService.java`、`FhirClientFactory.java`、`SmartBackendTokenService.java`、`PatientImportService.java`、`PatientSearchService.java`、`FhirSubscriptionService.java`、`FhirBulkExportService.java`、`TlsContextFactory.java`、`V28__ehr_integration.sql`、`application.yml`、`FhirImplementationGuideService.java`、`twcoreCatalog.json` 確認所有 endpoint / 欄位 / 預設值 / TW Core 版本均與程式碼一致；(b) 不憑空寫端點、不憑空寫欄位、不憑空寫版本號 — 全部來自實際代碼引用。**TFDA 追溯**：純文件 PR（安全性等級 A — 不改動程式碼、不影響臨床決策、不涉及 PHI），依規範不需建立設計/風險/驗證 Issue。 | docs/integration/HIS_INTEGRATION.md | |
| PAT-226 | 🔧 ci | 2026-09-18 | 前端 CI 與建置基底 Node.js 20 → 22（`ci.yml` 2 處、`regulatory-docs.yml`、未引用的 `docker/Dockerfile.frontend`、README / DEPLOYMENT_PASSWORD_RESET 字樣） | **脫離 EOL 的 Node 20，解 jsdom 30 相容** — Node 20 已於 2026-04-30 EOL；dependabot 的 jsdom 28 → 30.0.1（PR 556）在 CI 全部測試檔啟動即炸 `webidl.util.markAsUncloneable is not a function`，根因是 jsdom 30 `engines` 要 Node `^22.22.2 \|\| ^24.15.0 \|\| >=26`（相依 undici 8）。**修**：`setup-node` 三處 20 → 22。查證：正式前端映像由 compose / CI Docker Build / deploy.yml 以 `frontend/Dockerfile`（已是 node:26-alpine）建置，`docker/Dockerfile.frontend` 無任何引用，僅同步改 22 不留 EOL 字樣。**驗證**：本機 Node 22.23.2 直接跑 PR 556 分支（jsdom 30.0.1）123 檔 / 820 案全過；本 PR CI 的 Frontend Lint / Tests 在 Node 22 runner 上跑。合併後 PR 556 rebase 即可轉綠。選 22 與本機開發環境一致，升 24 只需同樣三處。無 schema / 程式碼變更。 | 需求 #861（A 級）；解 PR 556 |[`9503036`](../../commit/9503036) |
| BUG-140 | 🐛 fix | 2026-07-17 | 新增使用者驗證錯誤誤顯示為「帳號已被使用」(`TenantUsersPage` + `AdminUsersPage` 錯誤對應 + 前端密碼複雜度驗證) | **修新增員工「一直出現此帳號已被使用」— 400 驗證錯誤誤標成 409 重複** — 使用者回報:員工管理頁按新增員工一直跳「此帳號已經被使用」。**根因**(前端錯誤對應 bug,PAT-214 引入):`handleCreateUser` 把後端 **400(輸入驗證失敗)與 409(帳號重複)都對應成「帳號已被使用」**;且「建立」按鈕只檢查密碼長度 ≥ 8、不檢查複雜度。後端 `TenantCreateUserRequest.password` 需 `(?=.*[a-z])(?=.*[A-Z])(?=.*\d).{8,100}`,故長度足夠但缺大寫/小寫/數字的密碼可送出 → 後端 400(原因在回應 `details`:`password: must contain...`)→ 誤顯示「帳號已被使用」,換帳號重試仍失敗陷入迴圈。**修**:(1) 前端錯誤對應拆分 —— **409→「此帳號已被使用」;400→顯示 server `details`/`message` 實際驗證原因**,無則本地化 `invalidInput` 提示(密碼須含大小寫數字、帳號 3–50 字);其他→`message`/`createFailed`。(2) 前端加 `PASSWORD_RE` 對齊後端 regex —— 密碼欄位不符時**錯誤態 + 按鈕停用**,即時回饋、免無效往返。(3) **`AdminUsersPage` 同源既有缺陷一併修**(其 en 訊息原本甚至 hedge 成「Username already exists or invalid input」正是同一 conflation 的症狀)。i18n `admin.tenantUsers.errors.invalidInput` + `admin.users.errors.invalidInput`(en/zh-TW,並把 usernameExists 收斂回純粹重複語意)。**純前端修正,無後端 / schema 變更**(後端訊息本已正確,只是前端誤對應)。本機 fresh:前端 `tsc` 乾淨、`TenantUsersPage.test` 8(+3 回歸:弱密碼停用按鈕 / 400→非「帳號已被使用」/ 409→「帳號已被使用」)+ `AdminUsersPage.test` 12 綠。**TFDA 追溯**:#813(安全性等級 A,前端錯誤訊息對應修正,無 PHI / 無存取控制變更)。 | 使用者回報;錯誤訊息對應;純前端 | |
| PAT-225 | 🔧 ci | 2026-09-17 | changelog-backfill 自動 PR 改用 fine-grained PAT（`.github/workflows/changelog-backfill.yml`：checkout `token` 與 `GH_TOKEN` 改讀 `secrets.CHANGELOG_PAT \|\| secrets.GITHUB_TOKEN`） | **自動回填 PR 永遠 BLOCKED 的根因與修法** — 回填 workflow 用 `GITHUB_TOKEN` 推分支、開 PR，GitHub 規定該 token 產生的事件不再觸發其他 workflow（防遞迴）；main 的 ruleset 要求 TFDA / Frontend Lint / Frontend Tests / Backend Tests 四個 required checks 且無 bypass actor → 每個回填 PR（如 PR 848）只跑外部 GitGuardian、required checks 永遠缺席，只能人工往分支推空 commit 或 close + reopen 觸發。**修**：維護者建 fine-grained PAT（僅此 repo：Contents 讀寫 + Pull requests 讀寫）存為 `CHANGELOG_PAT`，workflow 以 `secrets.CHANGELOG_PAT \|\| secrets.GITHUB_TOKEN` 取 token；有 secret 時以維護者身分推送 / 開 PR，CI 正常觸發；secret 缺失或過期時退回 `GITHUB_TOKEN`（行為同現況，只是又要人工觸發），workflow 不會失敗。既有防迴圈判斷（推送者非 bot 且訊息以 `docs: backfill commit hashes` 開頭即跳過）在改用維護者身分後仍擋得住合併回填 PR 那一次 push。同步改 workflow 註解與自動 PR 內文說明。**驗證**：PyYAML 解析通過；真正的驗收是本 PR 合併後觸發的下一個回填 PR 自動出現全套 CI。無 schema / 程式碼變更。 | 需求 #858（A 級）；PAT 到期需重建 secret |[`9c450bc`](../../commit/9c450bc) |
| PAT-224 | ✨ patch | 2026-09-17 | 後端 Java 21 → 25 執行環境升版 + `jacoco-maven-plugin` 0.8.14（`backend/pom.xml`、`ci.yml` / `deploy.yml` / `regulatory-docs.yml`、README / DEPLOYMENT_GUIDE / backend/CLAUDE.md） | **收尾 VS Code Java 升版 agent（appmod）2026-08-27 的分支** — agent 只做到「改設定 + 編譯成功」（commit 訊息無 Tests 欄、未跑測試、未推遠端），本次驗證完成並補漏。**變更**：`<java.version>` 21 → 25、三個 workflow 的 `setup-java` 25、文件版本字樣（agent 另改了 `docker/Dockerfile.backend`，但該檔無任何引用；正式映像一直由 `backend/Dockerfile` 建置，執行階段早已是 `eclipse-temurin:25-jre-alpine`，PAT-227 更正並刪檔）；**新增 `jacoco-maven-plugin` 0.8.12 → 0.8.14**（0.8.12 不支援 Java 25 class 檔 major version 69、0.8.13 僅實驗性、0.8.14 正式支援；不升 CI 覆蓋率 agent 會炸）。**已查證**：Spring Boot 4.1 系統需求相容至 Java 26；兩個 temurin-25 映像存在於 Docker Hub；本機 JDK 25.0.2 `--release 25` fresh 編譯成功、class 檔 major 69。**驗證**：本機 JDK 25.0.2 完整後端套件 **1781 案、0 失敗、0 錯誤、1 略過**（PG-gated 整合測試，CI migration-test 跑）、jacoco 0.8.14 `prepare-agent` / `report` 正常（Analyzed 247 classes）、4 分 56 秒；CI Docker Build + Integration Smoke 31（`backend/Dockerfile`：temurin-26 建置 / 25-jre 執行）+ Migration Test 由 PR 驗。無 schema / migration / 程式碼變更；回退只需改回 21。正式機映像自帶 JRE，VM 不需裝 JDK。 | 需求 #856（A 級）；無 migration |[`6738bc5`](../../commit/6738bc5) |
| PAT-223 | ✨ feat | 2026-09-17 | 租戶隔離第二層：PostgreSQL Row-Level Security phase 1（`V70`、`TenantAwareDataSource`、`TenantContext.runWithRlsBypass`、`TenantRlsStartupCheck`、app role 分離、smoke / CI 在 RLS 下跑） | **忘了加 tenant 條件的 query 從「跨租戶漏資料」變成「查不到」** — 隔離原本完全靠 38 個 repository 檔手寫 `findByIdAndTenantId(...)`，BUG-131~139 逐一止血 50+ 例證明這條防線會一直破。本 PR 在 DB 層鋪第二層：**(1) V70 migration** 對 PHI 表 `measure_report`（+ `_group` / `_population` / `_stratifier` 經 join 繼承）、`test_case`（經父 measure 繼承）、`patient_import`、`ehr_connection` 啟用 RLS + `FORCE`，policy `tenant_isolation`：`app_rls_bypass() OR tenant_id = app_current_tenant()`（USING + WITH CHECK），helper `app_current_tenant()` 讀 `current_setting('app.tenant_id', true)`、空值即 NULL → 一列都不匹配（fail-closed）。**(2) `TenantAwareDataSource`**（BeanPostProcessor 包住 Hikari，僅 PostgreSQL）：每次取連線先 `set_config('app.tenant_id', …)` + `set_config('app.rls_bypass', …)`，值來自 `TenantContext`（無租戶 → default 租戶 id，與各 service 的 `effectiveTenantId()` 語意一致；default 不存在 → 空 → 看不到任何列）；session-level、每次 checkout 覆寫，pool 連線不會殘留前一位呼叫者的範圍；set 失敗連線直接關閉不外流。**(3) `TenantContext.runWithRlsBypass`**：跨租戶系統工作的唯一出口，目前只用在 `MeasureReportBackfillService` 啟動回填。**(4) 關鍵發現：Docker 的 `POSTGRES_USER` 是 superuser，superuser 無條件繞過 RLS** → 加入 app role 分離：`DB_APP_USERNAME` / `DB_APP_PASSWORD`（NOSUPERUSER / NOBYPASSRLS），`docker/postgres-init/10-app-role.sh` 在新 volume 首次初始化自動建立、既有 DB 可 `docker compose exec` 冪等重跑；backend datasource 用 app role、Flyway 改用自己的 DataSource 以 owner（`DB_USERNAME`）跑；compose / `.env.example` / `DEPLOYMENT_GUIDE §3.6` / 新 runbook `docs/runbooks/tenant-rls.md`。未設 DB_APP_* 時退回 owner（相容既有 .env），但 **(5) `TenantRlsStartupCheck`** 啟動時查 `pg_roles`（rolsuper / rolbypassrls）與 `pg_policies`，未生效就 `ERROR` + Prometheus gauge `tenant_rls_effective=0`；`TENANT_RLS_STRICT=true` 則拒絕啟動。**(6) 驗證**：`TenantAwareDataSourceTest`（6 案：tenant / default fallback + cache / 無 default → 空 / bypass / 旗標還原 / set 失敗關連線）；`TenantRlsIntegrationTest`（PG-gated，CI `migration-test` job 執行）以 NOSUPERUSER probe role 實證：租戶 A 的連線在**沒有 tenant 條件**的 query 下只看得到自己的 measure_report / test_case / population、無租戶看不到任何列、bypass 全看、對 B 的 INSERT 被 WITH CHECK 擋、UPDATE / DELETE 影響 0 列，並記錄 superuser 確實繞過（所以要 app role）；**smoke 全套 31 個 scenario 改在 app role + `TENANT_RLS_STRICT=true` 下跑**（本機 run.sh 與 CI smoke.yml 皆是），證明既有功能在 RLS 生效時零退化。**未動**：其餘 11 張 tenant 表（measure_definition、cds_*、cql_library、app_user、audit_log…）留 phase 2 逐批擴；`effectiveTenantId()` 23 份複製與 ArchUnit 規則另 PR。 | readiness / 租戶掃描的結構性收斂；V70 + 需 app role 部署步驟（§3.6） |[`9953a19`](../../commit/9953a19) |
| PAT-222 | ✨ patch | 2026-09-16 | 指標擁有權與生命週期收斂（`MeasureDefinitionService.create` / `update` / `createVersion`、`MeasureController.updateMeasure`、`MeasureLibrary` / `MeasureDetailsTab` 狀態欄、smoke 31） | **建立者即擁有者 + 狀態只能走審核流程** — 收 PAT-219 / BUG-143 留下的三個缺口。**(1) create 由伺服器蓋 ownerUsername**：UI 建指標的 payload 沒帶 owner，`create` 又只在 body 有帶時才設 → 從 UI 建的指標全部無擁有者；而 `OwnershipVerifier.verifyOwnership(null)` 是 fail-closed，**非 admin 連自己建的指標都不能 update / delete（403）**，審核通知也沒收件人（BUG-143）。現在 `create` 一律以當前使用者為 owner（body 給的 owner 忽略並 log；轉移只走 `/transfer`）、`createdBy` 補當前使用者。**(2) 狀態不可經 PUT 改**：原本 `update` 直接 `setStatus(body.status)`，任何能編輯的人都能把 draft 直接標 active（或把 active 退回 draft），PAT-219 只認 `active` 的守門形同虛設。現在 body status 與現況不同 → `ValidationException` 400，訊息指向 submit-for-review / approve / reject / retire；create 也一律從 draft 起（body 的 status 忽略；**FHIR 匯入的 active 指標同樣落 draft** —— 外部的 active 不等於本部署的核准）。**(3) createVersion 不再把來源版本無條件設成 active**（等於「建新版本」就靜默核准舊版）。**(4) update 的呼叫者身分**改由 controller 傳入 `currentUser`（原本拿 body 的 ownerUsername 當呼叫者做 lock 檢查，共享編輯者會判錯），audit 也記真正的操作者；body 的 ownerUsername 變更一律忽略並 log。**前端**：`MeasureLibrary` 編輯對話框與 `MeasureDetailsTab` 的狀態下拉改唯讀 + helper text（i18n en / zh-TW），`StatusChip` 與審核按鈕維持。**Smoke 31** 擴充：create 後 GET 斷言 `ownerUsername == admin`、PUT 改 status → 400 且訊息含 "review workflow"（新 lib `get-measure.sh` / `update-measure-raw.sh`）。**驗證**：`MeasureDefinitionServiceTest` +5（蓋 owner / 強制 draft；改 status 在 save 前被擋；同狀態可存但 owner 不變；lock 檢查用呼叫者；createVersion 不動來源狀態）、`MeasureControllerTest` +2（PUT 傳呼叫者；400 契約）。無 migration。**未動**：既有無擁有者的舊指標不回填（回填會把「只有 admin 能改」變成「只有 createdBy 能改」，屬資料政策另案）。 | 收 PAT-219 / BUG-143 殘餘；無 migration |[`e6866e0`](../../commit/e6866e0) |
| PAT-220 | ✨ patch | 2026-09-16 | smoke harness 進 CI（`.github/workflows/smoke.yml`；沿用 PAT-219 的 `SMOKE_SKIP_BUILD` / health timeout / `SMOKE_LOG_DIR` 開關） | **整合 smoke 從「記得 push 前跑」變成 PR gate** — `scripts/smoke/run.sh`（31 scenario）此前只在本機、靠紀律；CI 只有單元測試 / lint / migration / image build，BUG-110 / 111 / issue 230 這類「單元全綠但整合壞」的家族沒有自動閘門。新 workflow：PR 上只在 `backend/`、`docker/`、`scripts/smoke/` 或 workflow 本身有變更時跑（`git diff --name-only base...HEAD` 自帶閘門，不引第三方 action；其餘 PR 印 notice 後 skip），main push 一律跑；backend image 以 buildx + GHA cache（`scope=backend`，與既有 Docker Build job 共用）建一次並 `load`，harness 以 `SMOKE_SKIP_BUILD=1 BACKEND_IMAGE_TAG=smoke` 直接用；`docker/.env` 於 runner 以 `openssl rand` 產生一次性值（workflow 內無任何 secret 字面值）；backend / HAPI 健康逾時放寬到 180s（冷 runner）；`SMOKE_LOG_DIR` 的 backend / hapi-fhir / postgres log 一律以 artifact `smoke-stack-logs` 上傳（保留 7 天）。`timeout-minutes: 30`、`concurrency` 依 ref 取消舊跑。**首跑即揪出兩個 7 月租戶硬化後沒人跑全套而壞掉的 harness 問題**：(1) CDS scenario 10–18 全回「Service not found」—— BUG-139 起匿名 invoke 一律拒絕（backend log `CDS invoke denied ... (caller=null)`），`lib/invoke-cds.sh` 改帶 admin JWT（admin 是每個服務的 owner，BUG-142 保證可調用）；(2) scenario 24–31 撞 `User rate limit exceeded` 429（每用戶 DEFAULT 40 RPM，harness 幾分鐘內 150+ 呼叫），`compose.override.yml` 對 backend 放寬全部 `RATE_LIMIT_*`（含 yml 無 placeholder、靠 Spring relaxed binding 的 `RATE_LIMIT_CDSINVOKERPM` / `RATE_LIMIT_AUTHRPM`），只在 smoke overlay 生效、不動 prod、filter 仍執行。修後本機全套 31/31。**未動**：是否設為 branch protection required check 由維護者決定。 | 31 scenario 進 CI；每個後端相關 PR 約 +8–12 分鐘 runner；順手修好 7 月起壞掉的 CDS / 429 |[`9d7b8b0`](../../commit/9d7b8b0) |
| PAT-221 | ✨ patch | 2026-09-16 | 清 Security Scan 全域擋門的傳遞依賴 CVE（`backend/pom.xml`：`tomcat.version` 10.1.60、`httpcore5.version` 5.4.3、`libthrift` 0.24.0；`frontend/package.json` overrides `js-yaml` 4.3.2） | **Trivy DB 更新後每個 PR 的 Security Scan 都紅（PR 843 / 844 / 845 全中）** — backend 6 筆 + frontend 2 筆（backend 步驟過了 frontend 步驟才會跑，一次只看得到一層）全是傳遞依賴且皆有修復版：`tomcat-embed-core` 10.1.55 → **10.1.60**（CVE-2026-65182 CRITICAL security constraint bypass + 65905 + 68525；advisory 標修復版 10.1.58 但該版投票沒出貨、Central 上 10.1.57 直接跳 10.1.59，取現行 10.1.60；pom 既有 `tomcat.version` 覆寫）；`httpcore5` / `httpcore5-h2` 5.4.2 → **5.4.3**（CVE-2026-54399 / 54428 HIGH DoS；新增 `<httpcore5.version>` 覆寫 Boot 4.1 BOM 屬性，一次涵蓋兩個 artifact；路徑 hapi-fhir-structures-r4 → org.hl7.fhir.utilities → Saxon-HE → xmlresolver → httpclient5 5.6.1）；`libthrift` 0.22.0 → **0.24.0**（CVE-2026-43871 HIGH DoS infinite loop；dependencyManagement；路徑 hapi-fhir-structures-r4 → jena-shex 5.5.0 → jena-arq，平台不使用 RDF Thrift）。**frontend**：`js-yaml` 4.3.0 → **4.3.2**（CVE-2026-84375 HIGH DoS + GHSA-5p4m-2wfm-xmqj `!!omap` quadratic CPU；沿用 PAT-215 的 npm `overrides` 釘法，lock 只動 6 行）。依 PAT-215 / 217 做法升版不 VEX —— 三者皆可獨立升，沒有 hapi core 那種 AbstractMethodError 耦合。純依賴覆寫，無 schema / migration / 程式碼變更；Tomcat 留 10.1 線。本機 `dependency:tree` 確認解析到新版、`package` + 啟動測試成功；完整套件與 Security Scan 由 CI 驗。 | 解三個 PR 的 Security Scan 紅燈；無 migration |[`a81bade`](../../commit/a81bade) |
| BUG-143 | 🐛 fix | 2026-09-16 | 核准 / 退回無擁有者指標時通知建立失敗（`NotificationService.notifyMeasureApproved` / `notifyMeasureRejected`） | **UI 建立的指標核准不了 — approve / reject 回 409「A database constraint was violated」** — smoke `31-measure-status-guard`（PAT-219）第一次跑就撞到：`POST /api/measures/{id}/approve` 對沒有 `ownerUsername` 的指標，`approveMeasure` 更新狀態後呼叫 `notificationService.notifyMeasureApproved(approver, owner=null, …)`，`createNotification(null, …)` 撞 `notification.recipient NOT NULL` → `DataIntegrityViolationException` → 整個 `@Transactional` 回滾、狀態停在 in-review、API 回 409 且訊息誤導；`rejectMeasure` 同款。**影響範圍不只 API 客戶端**：前端 `MeasureLibrary` 建指標的 payload 只有 name / version / title / description / status / scoringType，**沒帶 ownerUsername**，而 `MeasureDefinitionService.create` 只在 body 有帶時才設擁有者 → 從 UI 建的指標全部無擁有者，審核流程走到 approve / reject 就炸。**修法**：`notifyMeasureApproved` / `notifyMeasureRejected` 對 null / blank owner 直接跳過（沒有收件人就沒有通知可發）並 log 一行；不動 `createNotification` 契約與其他通知。**未動（記於需求 issue）**：是否在 create 時由伺服器端以當前使用者補 `ownerUsername` —— 會改變 `requireOwnedMeasure` 的授權語意（無擁有者目前等於同租戶任何人可編輯 / 刪除），屬產品決策另案。**驗證**：新 `NotificationServiceTest`（null / 空 / 空白 owner 不碰 repository 與 WebSocket、不拋例外；有 owner 正常建立且 tenant 帶入；approver == owner 不自我通知）；smoke 31 的 approve 步驟由 409 變 200、狀態 active。無 migration。 | smoke 31 首跑即發現；PAT-219 同 PR | |
| PAT-219 | ✨ patch | 2026-09-16 | 指標評估生命週期守門（`MeasureStatusGuard` / `MeasureEvaluationService` / `CompositeMeasureService` / `GlobalExceptionHandler` / `MeasureEvaluationTab` / smoke `31`） | **未審核指標不得對真病人資料執行評估 — readiness review Critical #2 收尾** — `MeasureDefinition.status` 早有 `draft → in-review → active → retired` 生命週期與審核流程（submit / approve / reject / retire），但 `MeasureEvaluationService` 從不檢查 status：草稿、審核中、已停用的指標一樣能經 `$evaluate-measure`、批次評估、排程評估、複合指標元件去打真 FHIR server，CDS 警示 / 品質率 / dashboard 可能建立在未驗證的臨床邏輯上。**修法**：新增 `MeasureStatusGuard.requireEvaluable(id, def)`，在 `MeasureEvaluationService.evaluateMeasure(request, id, def)`（controller / batch / scheduled / composite 元件所有「儲存指標」評估路徑的共同漏斗）與 `CompositeMeasureService.evaluateComposite`（母指標）入口、於 counter / timer / 任何 FHIR / CQL 工作之前先檢查；非 `active` 一律拋新例外 `MeasureNotEvaluableException` → `GlobalExceptionHandler` 映射 **HTTP 409 `Measure Not Evaluable`**（狀態衝突而非驗證錯誤，獨立 error 標籤讓 EHR 整合端不用 parse 訊息就能分辨「尚未核准」與其他衝突），訊息指引改用測試案例或先送審核准。**刻意不擋**：inline CQL 的 ad-hoc 評估（`/api/measures/evaluate`、非數字 measureId，無儲存定義即 `def == null`）與測試案例沙盒（`TestCaseService` 走 in-memory bundle，不經 `MeasureEvaluationService`）—— 這兩條正是草稿邏輯該走的驗證路徑；`null` status 視同 draft（與 dashboard / pending-review filter / HQMF 既有處理一致）。批次評估既有 per-measure catch 會把被擋的指標記為 error 列、不中斷整批；排程遇到被停用的指標會記 `lastRunStatus=error`。**前端** `MeasureEvaluationTab`：已儲存且非 active 的指標顯示警示（附 `StatusChip`）並停用「評估指標」按鈕；未儲存的 ad-hoc 指標不受影響。**驗證**：`MeasureStatusGuardTest`（active 通過；draft / in-review / retired / 未知 / null 全擋且帶 id + status；id fallback；null def 放行）、`MeasureEvaluationServiceTest` +2（draft def 在觸碰 FHIR / CQL 前被擋 `verifyNoInteractions`；active def 正常走完）、`CompositeMeasureServiceTest`（新：draft 母指標在讀任何元件前被擋；active 走完 opportunity 聚合並驗證元件走 3-arg 受守門的 overload）、`MeasureControllerTest` +1（409 契約：`$.status` / `$.error` / `$.message`）、前端 `MeasureEvaluationTab.test.tsx`（新，5 案）；**smoke 新增 `31-measure-status-guard`**：`POST /api/measures` 建 draft → evaluate 回 409 + `Measure Not Evaluable` → submit-for-review + approve → 同一 evaluate 回 200、IP=4 / score=4.0，鎖住守門靠的是生命週期狀態而非 eCQM publish 路徑、且審核流程能在不重啟下解除（新 lib：`create-measure-definition.sh` / `evaluate-raw.sh` / `approve-measure.sh` / `assert-status-guard.sh`）。**殘餘風險（記於風險 issue）**：`PUT /api/measures/{id}` 仍接受 body 直接改 status、未走 `validateTransition`，擁有者可自行標 active 繞過審核 —— 審核流程強制屬獨立議題，不在本次範圍。無 migration。 | readiness review #2 Done；smoke +1（31）；無 migration | |
| PAT-218 | ✨ patch | 2026-08-03 | Spring Boot 4.0.x → 4.1.0 升版 + 明宣告 `LatencyUtils` 2.0.3(`backend/pom.xml`) | **提前完成 Boot minor 升版,避免 12 月後在 CVE 火警壓力下被迫升** — Spring Boot 4.0.x OSS 支援 **2026-12 到期**,本專案依賴 CVE 修補幾乎每月一次(BUG-121/123/125、PAT-215、PAT-217),每次低成本解法都是「同 minor 升 patch」;4.0.x 停止維護後此路徑消失。**Dependabot #601(bare bump 4.1.0)自 07-04 連續失敗 CI 一個月,根因查明**:Boot 4.1 BOM 將 micrometer 升至 **1.17.0**,`org.latencyutils:LatencyUtils` 不再被傳遞引入,但 `AbstractTimer` 仍引用 `org.LatencyUtils.PauseDetector` → `NoClassDefFoundError` → `MetricsConfig` 全部 Timer bean 於 context 啟動時失敗、整個 context 骨牌倒(CI 上看似數十個測試壞掉,實為單一根因)。**修法**:parent 4.0.7 → **4.1.0** + pom 明宣告 `LatencyUtils 2.0.3(runtime)`。**風險低於一般 minor 升版**:4.1.0 BOM 的 `spring-framework` 同為 **7.0.8**(與 PAT-217 後的正式機同版,framework 層零變化);主要變動為 micrometer 1.17.0 與 `spring-data-bom` 2025.1.6 → **2026.0.0**(→ `spring-data-commons` 4.1.0)。既有 CVE property 覆寫(tomcat/thymeleaf/spring-security/postgresql/jackson-bom)維持原值。過程 gotcha:升版後不清 `target` 直接跑測試會爆 100 個「cannot find symbol」假編譯錯誤(incremental compile 換 parent 後狀態失效,同 memory 已記錄模式),`rm -rf target/{classes,test-classes}` 後即正常。**純依賴版本升版,無 schema / migration / 應用程式碼變更**。本機 fresh:`dependency:list` 確認 framework 7.0.8 / data-commons 4.1.0 / micrometer 1.17.0 + LatencyUtils 2.0.3、完整後端套件見 PR CI。**TFDA 追溯**:#828(安全性等級 A,純依賴升版,無 PHI/授權/臨床邏輯變更)。 | 解 Dependabot #601 一個月紅燈;4.0 EOL 前從容升版;無 migration |[`27758ef`](../../commit/27758ef) |
| PAT-216 | ♻️ refactor | 2026-08-03 | 公開頁共用外框(新 `components/common/PublicLayout`/`PublicFooter`/`LanguageMenu` + 5 頁遷移 + `constants/layout.ts` 圓角常數) | **公開頁外框一致性收斂 + 修復 footer 導覽斷鏈** — 公開頁(未登入可瀏覽)隨 PAT-208~211 陸續新增而累積出兩套外框:LandingPage 為半透明浮動列疊在 hero 上,Learn/Docs/Templates/Status/Legal 為實心漸層列 + 返回鈕,且後五頁**各自複製一份幾乎相同的 header**(5 份重複)。**真正的缺陷在 footer**:僅 Landing 與 Learn 有 footer 且連結集合不同(Learn 缺內部導覽/聯絡信箱/CDS Hooks 連結),`/docs` `/templates` `/status` `/legal` **完全沒有 footer** → 使用者從首頁鑽進這些頁後,「服務條款」「隱私政策」入口整個消失,**申請帳號前應揭露的法律與隱私資訊變得不可及**。**收斂**:新增 `PublicLayout`(漸層 header + 返回鈕 + 可選 apply CTA + 語言鈕 + 內容 + footer;漸層抽成 `publicHeaderGradient` 單一來源)、`PublicFooter`(6 內部連結 + 4 外部 + 聯絡信箱 + copyright,**六個公開頁一致**)、`LanguageMenu`(原 Landing/Learn/Docs/Templates/Legal 各一份);五頁遷移至 `PublicLayout`,Landing 保留 hero 但共用 footer 與語言鈕(頁面各自持有 `<Container>`,因 LearnPage 需要 attribution 條與分頁軌的滿版佈局)。**圓角統一**:早期學習分頁與 landing 用 `borderRadius: 3`(30px)、後期新增的 eCQM/FHIRPath/語言參考分頁用 `2`(20px)、theme MuiPaper 預設 12px,切分頁時卡片形狀會跳 → 新增 `CARD_RADIUS`(20px)/`INNER_RADIUS`(10px) 於 `constants/layout.ts`,69 處硬編碼收斂 66 處(**保留 3 處刻意例外**:`CqlQuiz` 進度條膠囊、Landing/LoginForm 的 hero CTA 按鈕造型);另移除 8 處多餘 `Alert` 圓角覆寫、Chip 圓角覆寫與 `textTransform` 重複宣告(theme 本已提供)。順帶清除 `LearnPage` 的 `variant={isMobile ? 'scrollable' : 'scrollable'}` 死碼(連帶 `useMediaQuery`/`useTheme` 變未使用)。**新增 i18n** `landing.footer.learn`(en/zh-TW)。**純前端,無後端/schema/migration 變更**。本機 fresh:**tsc 乾淨**、完整前端套件 **812 過 / 3 失敗** —— 3 個失敗(`ForcePasswordChangeDialog`/`ClinicApplicationsAdminPage`/`ClinicApplyPage`)皆為 ~5000ms `waitFor` 滿載逾時,**隔離重跑 11/11 綠**,三檔均未被本 diff 觸及、亦不 import 任何新元件(同 PAT-212/PAT-215 記錄之既有 flake 家族)。淨減 321 行(25 檔 +145/−466)+ 新增 3 元件 246 行。**TFDA 追溯**:#824(安全性等級 A,呈現層一致性,無 PHI/授權/臨床邏輯變更)。 | 修復 4 頁 footer 缺失;5 份 header 重複收斂;無 migration |[`e8ff080`](../../commit/e8ff080) |
| PAT-217 | 🔒 fix | 2026-08-03 | 清 Security Scan 擋門的 Spring 依賴 CVE:`spring-boot-starter-parent` 4.0.6→4.0.7(`backend/pom.xml`) | **修四筆 Spring HIGH CVE、恢復 Security Scan gate 綠燈** — Trivy DB 更新後於既有後端依賴揭露四筆 HIGH:`spring-data-commons` 4.0.5 **CVE-2026-41695**(DoS)、`spring-expression` 7.0.7 **CVE-2026-41850**(DoS)、`spring-webmvc` 7.0.7 **CVE-2026-41842**(DoS)/**CVE-2026-41845**(XSS)。皆為**既有依賴之既有版本問題,非功能變更引入**;`Security Scan` 為 repo 層級 gate,會擋掉所有進 main 的 PR(本次即擋住純前端的 PAT-216 / PR #825)。**修法**:`spring-boot-starter-parent` 4.0.6 → **4.0.7** —— 其 BOM 已帶 `spring-framework` **7.0.8** 與 `spring-data-bom` **2025.1.6**(→ `spring-data-commons` **4.0.6**),**一次涵蓋四筆,不需逐一覆寫 property**;已以 `mvn dependency:list` 實測確認解析結果(commons 4.0.6、core/expression/webmvc 7.0.8)。pom 中既有的 CVE property 覆寫(`tomcat.version` / `thymeleaf.version` / `spring-security.version` / `postgresql.version` / `jackson-bom.version`)**不受影響、維持原值**。log 中另出現的 `org.hl7.fhir.*` 系列與 libthrift CVE 已在 `backend/.trivyignore` 完成 VEX 評估,非本次擋門項,**本案不新增任何 `.trivyignore` 條目**(四筆皆有上游修復版)。**純依賴版本 bump,無 schema / migration / 應用程式碼變更**。**TFDA 追溯**:#826(安全性等級 A,純依賴升版,無 PHI/授權/臨床邏輯變更)。 | 同 PAT-215 模式;解 PAT-216 的擋門;無 migration |[`dcb9e63`](../../commit/dcb9e63) |
| BUG-142 | 🐛 fix | 2026-07-24 | CDS 服務擁有者可調用自己的服務 + `toggleShared` 刷新記憶體登記表(`CdsHooksService.invokeService`/`toggleShared`) | **CDS 服務擁有者被鎖在自己服務外 + 切換 shared 不生效** — 實測調用 CDS 時,擁有者對自己建立的服務按調用卻回「service not available」,改 private 後仍無效。兩個後端缺陷:**(1) `toggleShared` 快取不一致** —— 只 `repository.save`(寫 DB)未更新 in-memory `serviceConfigs` map,而 `invokeService` 授權讀記憶體 config,故切換 shared 後 invoke 仍依舊值判斷、需重啟才生效(使用者「改 private 還是不行」);對照 `toggleServiceEnabled`/`updateService` 皆有 `serviceConfigs.put(...)`,`toggleShared` 漏了 → 補上比照的 synchronized put/remove。**(2) owner 授權漏洞** —— BUG-139 授權:shared 只認同租戶、private 只認擁有者;當擁有者租戶與服務標記租戶不一致(例:tenant=null 平台 admin 擁有被標成某租戶的服務),shared 分支只查租戶不查擁有者 → 擁有者被自己 shared 服務拒於門外(回與「找不到」同卡片)→ 授權加入「**擁有者永遠可調用自己的服務**(shared/private、不論租戶)」,**非擁有者的 shared→同租戶規則不變**(BUG-139 語意保留:caller=null 仍拒、private 非擁有者仍拒、legacy 無擁有者列維持寬鬆)。**由 BUG-141 修好調用頁下拉後,使用者實測調用 BMI 才暴露此二缺陷**(下拉能選 → 才碰到 invoke 授權層)。**純後端,無 schema/migration/前端變更**;owner-invoke 不擴大跨租戶暴露。本機 fresh:`CdsHooksServiceTest` 21 綠(+2 回歸:toggleShared 刷新後同租戶可 invoke、owner 跨租戶調用自己 shared 服務)、CdsInvocationService 10/Versioning 6/Controller 9 全綠。**TFDA 追溯**:#820(安全性等級 A,授權/快取一致性修正、無臨床邏輯或 PHI 變更)。 | BUG-141 暴露的 invoke 授權/快取缺陷;owner 可調用自己服務;無 migration |[`dfe6792`](../../commit/dfe6792) |
| BUG-141 | 🐛 fix | 2026-07-23 | CDS 調用/沙盒頁服務下拉來源修正(`InvokeServicePanel`/`SandboxPanel` 改吃 `useCdsServiceConfigs` + 過濾 enabled) | **CDS 服務調用/沙盒頁下拉選單永遠為空,新建服務無法選取** — 使用者新建 CDS 服務後,在「調用服務」(`InvokeServicePanel`)與「沙盒」(`SandboxPanel`)兩頁的**服務下拉選單永遠是空的**,選不到任何服務、無法測試調用。**根因為 BUG-139(#775/#776)回歸**:BUG-139 為租戶隔離把後端匿名 discovery 面 `CdsHooksService.getSharedServiceDefinitions()` 改為 `return List.of()`(永遠空),改走認證後 tenant-scoped 面;但前端兩頁下拉仍透過 `useCdsServices()` → `discover()` → `GET /cds-services/` → `discovery()` → `getSharedServiceDefinitions()` 這條**已被清空**的 endpoint 取清單 → 下拉無條件為空(與服務 enabled/owner 無關)。真正 tenant-scoped 的 `useCdsServiceConfigs()`(`GET /cds/services`)其實已在 `InvokeServicePanel` 載入,卻僅用於自動載 CQL 到編輯器、未填下拉。**修正**:兩頁下拉改由 `useCdsServiceConfigs()`(認證後 tenant-scoped)提供,並**僅列 `enabled` 服務**(後端 in-memory registry 只持有 enabled,選到 disabled 只會回「Service not found」卡)；`InvokeServicePanel` 一併把 loading/error 態接到 configs query;移除已無用的 `useCdsServices`/`CdsServiceDefinition` import。**方向與 BUG-139 租戶隔離用意一致,不重啟匿名跨租戶 discovery**。**純前端,無後端/schema/migration 變更**(後端行為維持 BUG-139 現狀)。本機 fresh:前端 tsc 乾淨、CDS 元件測試 10 綠(含 2 新 BUG-141 回歸:下拉由 configs 來源解析 svc-1、config 來源服務可 invoke);SandboxPanel 無專屬測試(CdsPanel.test 將其 stub)。**TFDA 追溯**:#816(安全性等級 A,純前端下拉資料來源修正、無 PHI/授權變更)。 | BUG-139 前端遺留缺口;調用頁下拉修復;無 migration |[`4266419`](../../commit/4266419) |
| PAT-215 | 🔒 fix | 2026-07-23 | 清除擋 Security Scan gate 的既有依賴 CVE:pgjdbc 42.7.11→42.7.12 + pin jackson-core 2.x 2.21.4(`backend/pom.xml`)+ 前端 axios 1.16.1→1.18.1(`frontend/package.json`) | **修三個既有依賴漏洞、恢復 Trivy Security Scan gate 綠燈** —(1)後端 `org.postgresql:postgresql` 42.7.11 → **42.7.12**(CVE-2026-54291,經 `com.ongres.scram/scram-client`);(2)後端 `com.fasterxml.jackson.core:jackson-core`(2.x)2.21.2 → **2.21.4**(GHSA-r7wm-3cxj-wff9,Async parser maxNumberLength bypass)—— Spring Boot 4.0.6 BOM 把 jackson-core 壓在 2.21.2,既有 `jackson-databind` 直接依賴 pin 2.21.4 不會連帶提升 core,故補一條 jackson-core 直接依賴 pin;(3)前端一波:`axios` `^1.16.1` → **`^1.18.0`**(鎖 1.18.1;GHSA-gcfj-64vw-6mp9)、`js-yaml` override **`^4.3.0`**(鎖 4.3.0;CVE-2026-59869 YAML DoS,傳遞依賴故走 `overrides`)、`npm audit fix`(非破壞性)清掉其餘 HIGH transitive advisory(build 工具鏈:postcss 8.5.22 / esbuild 0.28.1 / picomatch / brace-expansion / @babel/core 等,不 ship 到 prod runtime 但 Trivy 以 HIGH 擋門),lock 淨動 ~858 行。皆**既有依賴問題(非功能變更引入)**,會擋所有 PR 的 Security Scan gate(分 backend + frontend 兩步),於 #817(BUG-141)重跑時被 429 flake 消退後逐一揭露(Trivy 每輪僅露一筆)。既有修復版可用故採升版/override 而非 `.trivyignore` VEX 忽略。**純依賴版本 bump,無 schema / migration / 應用程式碼變更**;**前端 tsc 乾淨、vite build 綠、測試 812 過**(1 個 `ClinicApplyPage` 為滿載逾時 flake,隔離重跑 34.98s 綠、與本案無關)。**殘留**:1 個 moderate `dompurify`(經 `monaco-editor` 釘住,需破壞性 major 升版)未動 —— moderate 不觸 Trivy HIGH/CRITICAL gate,留待 monaco 升版一併處理。**TFDA 追溯**:#818(安全性等級 A,純依賴升版、無 PHI/邏輯變更)。 | 清 pgjdbc+jackson-core+axios+js-yaml+build 鏈 HIGH CVE;恢復 Security Scan gate;無 migration |[`6b48a31`](../../commit/6b48a31) |
| PAT-214 | ✨ feat | 2026-07-17 | 診所 ADMIN 租戶範圍員工管理面(新 `TenantUserService`/`TenantUserController` `/api/tenant/users` + 2 DTO + 前端 `TenantUsersPage`/`tenantUserApi` + Header nav) | **補 BUG-131 遺留缺口:診所 ADMIN 的租戶範圍員工管理** — BUG-131 為止血「診所 ADMIN 看得到全平台使用者」把所有使用者管理端點鎖成平台操作員限定,但**未補租戶範圍等價功能** —— 診所租戶 ADMIN(如 alumi001)開通後**完全無法管理自己診所員工**(不能建帳號 / 設角色 / 停用 / 重設密碼,只能改自己密碼 + 看自己稽核)。**補上租戶範圍員工管理面**:診所 ADMIN 可管理**且僅能管理自己租戶內**使用者。**後端** `TenantUserService`(全操作限縮 `effectiveTenantId()`;`requireSameTenant` 對每個目標使用者再驗租戶,他院使用者**回 not-found 而非 forbidden**——防以連號 id 枚舉探測他院帳號,沿用 BUG-133~138 一貫作法;`requireNotSelf` 自我護欄——不可對自己停用/降級,避免鎖死唯一 admin + 使當前 session 失效;改角色/停用即時 `bumpVersion`+`revokeAllForUser`,停用另撤 API key)+ `TenantUserController` `/api/tenant/users`(class `hasRole('ADMIN')`,落 SecurityConfig `/api/**`→authenticated,**無需改 SecurityConfig**)+ 2 DTO(`TenantCreateUserRequest`/`TenantRoleUpdateRequest`,role pattern 放寬 `ADMIN|USER|DEPARTMENT_ADMIN`——既有 Admin 版僅 `ADMIN|USER`)。**使用者決策**:診所 ADMIN 於自己租戶內**可指派全部三種角色(含 ADMIN)**——等同租戶擁有者;跨租戶重新指派仍限平台操作員(`TenantService.assignUser`,本面不提供)。重設密碼回**一次性 setup 連結**(沿用診所開通審核流程),SMTP 未配置亦可用(admin 轉交)。**前端** `TenantUsersPage`(`/tenant/users`,AdminRoute):員工表 + 建立對話框 + 三角色下拉 + 啟用開關 + 重設密碼(顯示 setup 連結、一次性、可複製、warning)+ `tenantUserApi`;`Header` nav:**非平台操作員的 ADMIN** 顯示「員工管理」(平台操作員維持全平台 Users 頁不重複);i18n `admin.tenantUsers.*` + `common.nav.tenantUsers`(en/zh-TW)。**無 schema/migration**(沿用 `UserRepository.findByTenantId`、`user.tenantId`、TokenVersion/RefreshToken 撤銷慣例)。本機 fresh **後端全套 1751 案綠**(較 PAT-213 之 1736 +15:`TenantUserServiceTest` 10〔診所不併 null-tenant / 烙租戶+forcePasswordChange / **跨租戶回 not-found** / **自我護欄** / 改角色撤 session / 停用撤 session+key / setup 連結〕+ `TenantUserControllerTest` 5〔ADMIN 200 / USER 403 / DEPARTMENT_ADMIN 403 / create 路由 / 非法角色 400〕)+ **前端 tsc 乾淨**、`TenantUsersPage.test` 5、`Header.test` 5(含新 tenantUsers nav 斷言,平台操作員不見)。**安全模型與 BUG-133~138 一致**:邊界在租戶化查詢+再驗,不在角色。**TFDA 追溯**:#808(安全性等級 B)/設計 #809/風險 #810/驗證 #811。 | 補 BUG-131 缺口;租戶內員工管理;無 migration | |
| PAT-213 | ✨ feat | 2026-07-17 | 每租戶彙總速率限制(新 `TenantRateLimitFilter` + `RateLimitProperties` tenant* + `SecurityConfig` filter 鏈 + application.yml) | **多租戶開通閉環收尾:每租戶彙總速率限制(#700 系列最後一塊)** — 診所自助開通(PAT-203/204)上線後平台成多租戶共用,但速率限制只有每 IP(`RateLimitFilter`)與每使用者(`UserRateLimitFilter`)兩層。**缺口**:一家診所多員工 / 多 API key 的**合計**流量可在單一 tier(translate/execute)耗盡共用容量,拖垮其他租戶(吵鬧鄰居 / 租戶級 DoS)—— changelog PAT-203/204 明列「per-tenant rate limit 為後續 PR」。**新增第三層** `TenantRateLimitFilter`:以「租戶 id + tier」為 bucket key,同租戶**所有使用者共用一桶**(合計計量),不同租戶彼此隔離;租戶 id 取自 `TenantContext`(由 `JwtAuthenticationFilter` 從 JWT 租戶 claim / API-key 擁有者租戶填入),故 filter 排在 JWT 認證之後(`jwt → userRateLimit → tenantRateLimit → audit`)。**無租戶脈絡的請求直接放行**(匿名 / legacy 平台帳號交由既有 IP + 每使用者兩層)。重用 `RateLimitFilter` 既有 `TokenBucket`/`RateTier`/`resolveTier`,tier 語意三層一致。超限回 **429 + `Retry-After` + `X-TenantRateLimit-*` headers + `rate_limit_exceeded{layer=tenant}` metric**。`RateLimitProperties` 加 `tenantLimitEnabled`(預設 true)+ `tenantTranslate/Execute/FixSuggestion/DefaultRpm`(60/32/12/160,皆高於對應每使用者上限反映多員工合計,`RATE_LIMIT_TENANT_*` 環境變數可覆寫);application.yml 同步。**僅新增防禦性可用性控制,不放寬既有限制、不觸 PHI/授權、無 schema/migration**。本機 fresh **後端全套 1736 案綠**(較 PAT-212 之 1729 +7;速率限制三檔合計 32 案:`TenantRateLimitFilterTest` 7〔無租戶放行 / 合計計量 / 租戶隔離 / 停用 / 429+headers+metric〕+ `UserRateLimitFilterTest` 7 + `RateLimitFilterTest` 18)。**殘留**:記憶體 TokenBucket 為 per-instance(與既有兩層同限制,多實例下非全域,單實例部署足夠);租戶級上限為靜態預設,per-tenant 客製方案為後續。**至此 #700 多租戶開通閉環(申請→審核→開通→設定密碼→登入進隔離租戶→租戶級配額)全數完成**。**TFDA 追溯**:#803(安全性等級 B)/設計 #804/風險 #805/驗證 #806。 | 開通閉環收尾;租戶級 DoS 防護;無 migration | |
| PAT-212 | 🔒 fix | 2026-07-17 | FhirController 目標伺服器由任意 `fhirServer` URL 收斂為租戶範圍 `connectionId`(後端 8 端點 + `FhirDataProviderService`/`FhirBulkExportService` + 前端 FHIR 瀏覽器/假病人上傳) | **租戶存取控制掃描收尾:FhirController 裸 fhirServer 網址 → tenant-scoped connectionId(API 合約變更)** — 掃描降級待辦的 7 例根因:`/api/fhir/*` 八端點(搜尋/讀取/建立/更新/刪除/`$transaction`/`$export`/`$search-by-demographics`)與假病人 Bundle 上傳,原本以**自由輸入的 `fhirServer` URL** 指定目標 FHIR 伺服器 —— 任何登入者可把平台 FHIR CRUD/交易/批次匯出**指向任意 URL(SSRF 風格外連)**,且**完全繞過租戶邊界**把院內臨床資料交易導向未授權伺服器。**收斂**:移除任意 URL 入口,目標一律以**租戶範圍 EHR `connectionId`** 指定,經既有 tenant-scoped `getById`(`findByIdAndTenantId`,他院 id→not-found)解析;未指定(null)→平台共用沙盒(匿名用戶端,行為同收斂前預設);有連線→該連線的認證用戶端(`createAuthenticatedClient`)。**後端**:`FhirController` 注入 `EhrConnectionService` + `resolveConnection`(connectionId→getById/null→沙盒),八端點 `fhirServer:String`→`connectionId:Long`,移除 `InputValidator.requireValidUrl`;`FhirDataProviderService` 六方法(+fallback)首參 String URL→`EhrConnectionEntity` + `resolveClient`(認證/沙盒二選一);**`executeTransaction` 保留 String 版**(CQL 執行內部 `batchFetchPatientResources` 仍呼叫)**+ 新增 `executeTransactionForConnection`**;`FhirBulkExportService.kickOffExport` 連線化 + 注入 `FhirClientFactory`;`GlobalExceptionHandler` 加 `MethodArgumentTypeMismatchException`→400(`connectionId=abc` 屬用戶端錯誤,原落通用 500)。**前端**:`FhirBrowser` 自由輸入 URL 欄(`FhirServerUrlField`)→**租戶範圍 EHR 連線下拉**(`ehrApi.getConnections`,沙盒預設),prop `fhirServer:string`→`connectionId:number|null` 貫穿 Search/Read/Transaction/BulkExport/ResourceDetail/ResourceEditor;`fhirApi` 7 方法 + `BulkExportParams` + `useFhirQueryHistory` 去重鍵同改;**假病人產生器 `GenerationResultPanel` 上傳目標同改連線下拉(關掉其任意 URL 上傳口)**;i18n en/zh-TW(`fhir.browser.connectionLabel`/`sandboxOption`、`patientGenerator.result.uploadServerLabel` 改義 + `uploadSandboxOption`)。**內部 CQL 執行 FHIR 抓取路徑維持 String URL**(引擎資料來源、非使用者可控對外參數,不在收斂範圍)。**無 schema/migration/資料表變更**。本機 fresh **後端全套 1729 案綠**(FhirControllerTest 28:**+回歸 connectionId 走 tenant-scoped getById**、任意 fhirServer 被忽略;FhirControllerErrorTest 5:URL 驗證測試改 connectionId 型別不符→400;FhirFallbackTest 4/FhirBulkExportServiceTest 2 簽章更新)+ **前端 tsc 乾淨**、FHIR/假病人/hooks 測試綠(全套 808,2 個 `ClinicApplyPage` 逾時為滿載機器 flaky,**隔離重跑 2/2 綠、與本案無關且不在本 diff**)。**過程誠實記錄**:`executeTransaction` 因 CQL 執行內部呼叫無法改簽章 → 保留 String 版 + 新增 connection 版;blanket replace 曾誤傷 `createClient` 其他呼叫點 → 改逐方法精準替換。**曝險已查證**:生產 `ehr_connection=0`(暫無真實連線標的)→ 收斂為預防性。**至此租戶存取控制掃描 49 例全數關閉**:42 已修(BUG-132~138)+ 7 FhirController 降級項於本案 connectionId 收斂。**TFDA 追溯**:#798(安全性等級 B)/設計 #799/風險 #800/驗證 #801。 | 存取控制掃描收尾;API 合約變更;掃描 49 例全關 | |
| PAT-211 | ✨ feat | 2026-07-17 | 服務條款 + 隱私政策頁框架(`LegalPage` `/terms` `/privacy` + LandingPage footer) | **#8 對外門面第四、五塊(合併):服務條款 + 隱私政策頁框架** — 法律內容須經法務審閱,故本案僅建**框架 + 草稿佔位**。共用 `LegalPage`(prop `doc: 'terms'|'privacy'`):`/terms`(服務說明/資格/使用者責任/智財/免責/責任限制/變更/準據法)、`/privacy`(蒐集資料/使用目的/PHI 處理/保存/分享/安全/使用者權利/聯絡)。**每頁頂端明顯 warning banner:「草稿,待法務審閱,不構成正式或具約束力之法律條款」**;各章節 body 為明確佔位(「正式內容待法務審閱後補上」),**不捏造具約束力法律文字**。待正式文字備妥只需替換 i18n、不需改結構。LandingPage footer 加條款、隱私連結(至此 footer 含文件/範本庫/狀態/條款/隱私 5 個公開頁)。i18n `landing.legal.*`(en/zh-TW,各 +78 行)。**無 PHI、無後端變更**。本機 fresh:前端 tsc 乾淨、`LegalPage.test` 2 案(**關鍵斷言:條款/隱私兩頁皆顯示草稿 banner**,佔位內容不會被誤認為正式條款)。**至此 #8 對外門面 5 塊全部完成**(範本庫/status/文件中心/條款/隱私)。**TFDA 追溯**:#796(安全性等級 A,草稿佔位頁,無 PHI)。 | 對外門面 4-5/5;草稿佔位;#8 完成 | |
| PAT-210 | ✨ feat | 2026-07-17 | 平台使用文件中心(`DocsPage` `/docs` + LandingPage footer) | **#8 對外門面第三塊:平台使用文件中心** — 既有「學習中心」`/learn` 教 **CQL 語言本身**,本頁補上**平台產品使用文件**。公開 `/docs`(免登入,lazy):快速開始(4 步:申請→開通→匯入→建立指標/CDS)、平台功能卡片(CQL Builder / 品質量測 / CDS Hooks / FHIR 匯入 / 假病人 / 術語,導向 /learn 或 /templates)、整合(CDS Hooks per-user 端點 / API Key / 健康存摺 FHIR 上傳)、資源連結(/learn、/templates、/status + CQL/TWCore/CDS Hooks 外部規範)。與 /learn 明確區隔不重複。LandingPage footer 加文件連結。i18n `landing.docs.*`(en/zh-TW,各 +85 行,格式對齊無全檔重排)。**無 PHI、無後端變更**。本機 fresh:前端 tsc 乾淨、`DocsPage.test` 2 案(區塊 + 4 步驟);公開頁測試(範本庫/status/docs)共 6 案綠。**TFDA 追溯**:#794(安全性等級 A,平台功能事實說明頁,無 PHI)。 | 對外門面 3/5;平台文件;無後端 | |
| PAT-209 | ✨ feat | 2026-07-17 | 公開服務狀態頁(`StatusController` `/api/status` + `StatusPage` `/status` + LandingPage footer) | **#8 對外門面第二塊:公開服務狀態頁** — 使用者無法用平台時可不登入查看是平台中斷還是自身問題。**後端** `GET /api/status`(SecurityConfig permitAll,匿名):回傳整體狀態(operational/degraded)+ 元件(api、database)reachability + 時間戳;資料庫 check 用 `DataSource.getConnection().isValid(2)`(不揭露任何資料/數量)。**刻意粗略,不揭露內部細節/版本/PHI**。**前端** `/status`(免登入,lazy):整體狀態綠/紅卡 + 元件清單 + 最後檢查時間,React Query 30s 自動輪詢;請求失敗顯示 outage。LandingPage footer 加內部連結列(範本庫 / 狀態,為後續文件/條款/隱私鋪路)。i18n `landing.status.*`(en/zh-TW)。本機 fresh:後端 **全套 1728 案綠**(StatusControllerTest 1:公開 + DB 通時 operational)+ 前端 tsc 乾淨、StatusPage.test 2 案(operational / 請求失敗→outage)。**TFDA 追溯**:#792(安全性等級 A,公開狀態端點僅回粗略 reachability、無敏感資料)。 | 對外門面 2/5;公開狀態端點 | |
| PAT-208 | ✨ feat | 2026-07-16 | 官方 CQL 範本庫公開頁(`TemplateCatalogPage` + 路由 `/templates` + LandingPage 連結) | **#8 對外門面第一塊:官方 CQL 範本庫公開頁** — 診所來到 twcql.com 可在登入/申請前瀏覽現成 CQL 範本,支持「隨插即用」評估。公開路由 `/templates`(免登入,lazy-loaded),把內建的 TWCDI 範本(TW Core Data for Interoperability,`twcdiTemplates.json`,已有 name/category/description/cql metadata)依 category 分組顯示,每筆可展開/複製 CQL(`useCopyToClipboard`)。沿用公開頁 gradient header + 語言切換 + 申請 CTA;LandingPage header 加「範本庫」連結。i18n `landing.catalog.*`(en/zh-TW)。**無 PHI、無資料異動、無後端變更**。**順帶**:`test-utils` 加 `HelmetProvider`(範本庫用 Helmet 做 SEO meta,additive 無害,也讓其他 Helmet 頁可測)。本機 fresh:前端 tsc 乾淨、`TemplateCatalogPage.test` 2 案 + 全部 page 測試 16 檔 99 案綠(確認 test-utils 改動安全)。**TFDA 追溯**:#790(安全性等級 A,低風險公開內容頁)。 | 對外門面 1/5;公開頁;無後端 | |
| PAT-207 | ✨ feat | 2026-07-16 | 上傳 bundle 大小上限修正 + entry 上限 + 可選 TW Core 驗證(PAT-206 殘留收尾) | **收尾 PAT-206 風險 #783 明列的兩項殘留** — (1) **上傳大小上限**:部署後發現既有全域 multipart `max-file-size` 僅 **1MB**,而真實健康存摺 bundle 動輒數 MB **會被擋掉** —— 功能對真實資料形同虛設。提高至 `${MULTIPART_MAX_FILE_SIZE:10MB}`/12MB(可環境變數覆寫);`PatientImportService.importUploadedBundle` 加 entry 數上限 10000(超過→400 不落資料,防禦大 bundle 記憶體)。(2) **可選 TW Core v1.0.0 驗證**:端點加 `validate=false`(預設),為 true 時匯入後呼叫 `FhirValidationService.validateBundle` (用已載入 IG 逐 resource 驗證)回傳一致性計數,**非阻擋**(匯入已完成)。**設可選而非強制**:逐 resource 驗證對大 bundle 慢(數十秒),不該每次強制承受。新增 `FhirBundleImportResult` DTO(patientImport + validated + total/valid/invalid 計數);端點回傳型別改此 DTO(向後相容,原欄位在 patientImport 內);**service 方法回傳型別不變**(驗證組裝在 controller,既有 4 個 service 測試不動)。前端 `FhirBundleUpload` 加「驗證 TW Core 一致性」checkbox + 顯示「N/M 筆符合 TW Core」(有 invalid 時 warning);`connectionId` 型別改 optional(上傳來源為 null)+ `source` 欄位。無 schema/migration。本機 fresh **後端 1727 案綠**(PatientImportServiceTest 5:+entry 上限)+ 前端 tsc 乾淨、FhirPage/ehr 9 綠。**過程記錄**:entry 上限測試初版用空 resource 作 entry,因 **HAPI JSON parser 丟棄無內容 resource**(hasEntry 變 false)誤觸空 bundle 分支,給每個 Patient 加 id 使其非空後通過 —— HAPI parser 行為,對真實有內容 bundle 無影響。**殘留**:10MB 為全域上限、未做大檔記憶體壓力實測、validate=true 極大 bundle 理論上可長佔執行緒(靠 entry 上限 + 預設關閉緩解)。**TFDA 追溯**:#785(安全性等級 B)/設計 #786/驗證 #787/風險 #788。 | PAT-206 收尾;1MB→10MB;驗證可選 | |
