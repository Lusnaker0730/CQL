...
|-----|------|------|------|------|------|--------|
| PAT-049 | ✨ patch | 2026-03-13 | 前端（表單驗證） | 前後端欄位約束對齊 — fieldConstraints.ts 共用常數 + 6 表單元件 maxLength 對齊後端 @Size + 長文字欄位字數計數 + StringField/TextAreaField maxLength prop | 防止前端超長輸入導致 400 | |
