...
|-----|------|------|------|------|------|--------|
| PAT-215 | 🔒 fix | 2026-07-23 | 升 PostgreSQL JDBC driver 42.7.11 → 42.7.12(`backend/pom.xml` `postgresql.version`) | **修 CVE-2026-54291(pgjdbc scram-client 認證用戶端漏洞)** — Trivy CI Security Scan 揭露後端依賴 `org.postgresql:postgresql` 42.7.11 存在 HIGH/CRITICAL 漏洞 CVE-2026-54291(經由 `com.ongres.scram/scram-client`),已於 42.7.12 修復。此為既有依賴問題(非功能變更引入),且會擋所有 PR 的 Security Scan gate。升版清除漏洞、恢復 gate 綠燈。既有修復版可用,故採升版而非 `.trivyignore` VEX 忽略(沿用上一輪 42.7.10→42.7.11 修 CVE-2026-42198 的作法)。**純依賴 patch 版本 bump,無 schema / migration / 應用程式碼變更**;42.7.12 於 Maven Central 存在(jar+pom 200),CI Backend Tests / Docker Build / PostgreSQL Migration Test 驗證資料層不受影響。**TFDA 追溯**:#818(安全性等級 A,純依賴升版、無 PHI/邏輯變更)。 | 修 pgjdbc CVE;恢復 Security Scan gate;無 migration | |
