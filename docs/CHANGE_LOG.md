...
|-----|------|------|------|------|------|--------|
| PAT-215 | 🔒 fix | 2026-07-23 | 清除擋 Security Scan gate 的既有依賴 CVE:pgjdbc 42.7.11→42.7.12 + pin jackson-core 2.x 2.21.4(`backend/pom.xml`)+ 前端 axios 1.16.1→1.18.1(`frontend/package.json`) | **修三個既有依賴漏洞、恢復 Trivy Security Scan gate 綠燈** —(1)後端 `org.postgresql:postgresql` 42.7.11 → **42.7.12**(CVE-2026-54291,經 `com.ongres.scram/scram-client`);(2)後端 `com.fasterxml.jackson.core:jackson-core`(2.x)2.21.2 → **2.21.4**(GHSA-r7wm-3cxj-wff9,Async parser maxNumberLength bypass)—— Spring Boot 4.0.6 BOM 把 jackson-core 壓在 2.21.2,既有 `jackson-databind` 直接依賴 pin 2.21.4 不會連帶提升 core,故補一條 jackson-core 直接依賴 pin;(3)前端 `axios` `^1.16.1` → **`^1.18.0`**(鎖到 1.18.1;GHSA-gcfj-64vw-6mp9,Node HTTP adapter 繼承 proxy 誤用)。三者皆**既有依賴問題(非功能變更引入)**,會擋所有 PR 的 Security Scan gate(分 backend + frontend 兩步),於 #817(BUG-141)重跑時被 429 flake 消退後逐一揭露。既有修復版可用故採升版而非 `.trivyignore` VEX 忽略(沿用 42.7.10→42.7.11 修 CVE-2026-42198 慣例)。**純依賴版本 bump,無 schema / migration / 應用程式碼變更**;42.7.12、jackson-core 2.21.4、axios 1.18.1 於各自 registry 皆存在,前端 tsc 乾淨,CI Backend Tests / Docker Build / PostgreSQL Migration Test / Frontend Tests 驗證不受影響。**TFDA 追溯**:#818(安全性等級 A,純依賴升版、無 PHI/邏輯變更)。 | 清 pgjdbc + jackson-core + axios CVE;恢復 Security Scan gate;無 migration | |
